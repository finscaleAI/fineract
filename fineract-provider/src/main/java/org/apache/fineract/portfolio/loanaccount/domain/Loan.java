...
public class Loan extends AbstractPersistableCustom {
  ...
  @Column(name = "loan_type_enum", nullable = false) private Integer loanType;
  ...
  @Column(name = "loan_status_id", nullable = false) private Integer loanStatus;
  ...
  @Column(name = "loan_counter") private Integer loanCounter;

  @Column(name = "loan_product_counter") private Integer loanProductCounter;
  ...
  @Column(name = "approved_principal", scale = 6, precision = 19,
          nullable = false)
  ...
  @JoinTable(name = "m_loan_rate", joinColumns = @JoinColumn(name = "loan_id"),
             inverseJoinColumns = @JoinColumn(name = "rate_id"))
  private List<Rate> rates;

  public static Loan newIndividualLoanApplication(
      final String accountNo, final Client client, final Integer loanType,
    ...
    return new Loan(accountNo, client, group, loanType, fund, officer,
                    ...
                    loanRepaymentScheduleDetail, status, loanCharges,
                    collateral, syncDisbursementWithMeeting, fixedEmiAmount,
  ...
  public static Loan newGroupLoanApplication(
      final String accountNo, final Group group, final Integer loanType,
      final LoanProduct loanProduct, final Fund fund, final Staff officer,
      final CodeValue loanPurpose,
      final LoanTransactionProcessingStrategy transactionProcessingStrategy,
      final LoanProductRelatedDetail loanRepaymentScheduleDetail,
  ...
  public static Loan newIndividualLoanApplicationFromGroup(
      final String accountNo, final Client client, final Group group,
      final Integer loanType, final LoanProduct loanProduct, final Fund fund,
  ...
  protected Loan() { this.client = null; }

  private Loan(
      final String accountNo, final Client client, final Group group,
      final Integer loanType, final Fund fund, final Staff loanOfficer,
      final CodeValue loanPurpose,
      final LoanTransactionProcessingStrategy transactionProcessingStrategy,
     ...
     * principal_amount, approved_principal and principal_amount_demanded will
     * same amount and that amount is same as applicant loan demanded amount.
     */
  ...
  private LoanSummary updateSummaryWithTotalFeeChargesDueAtDisbursement(
    ...
    if (this.summary == null) {
      ...
      this.summary.updateTotalFeeChargesDueAtDisbursement(
  ...
  private Set<LoanCharge>
  associateChargesWithThisLoan(final Set<LoanCharge> loanCharges) {
  ...
  private Set<LoanCollateral>
  associateWithThisLoan(final Set<LoanCollateral> collateral) {
  ...
  public void setAccountNumberRequiresAutoGeneration(
    ...
    this.accountNumberRequiresAutoGeneration =
  ...
  public void addLoanCharge(final LoanCharge loanCharge) {
    ...
    if (isChargesAdditionAllowed() && loanCharge.isDueAtDisbursement()) {
      ...
      final String defaultUserMessage =
          "This charge which is due at disbursement cannot be added as the loan is already disbursed.";
      throw new LoanChargeCannotBeAddedException(
          "loanCharge", "due.at.disbursement.and.loan.is.disbursed",
          defaultUserMessage, getId(), loanCharge.name());
    }

    validateChargeHasValidSpecifiedDateIfApplicable(
        loanCharge, getDisbursementDate(),
  ...
  public ChangedTransactionDetail reprocessTransactions() {
    ...
    final LoanRepaymentScheduleTransactionProcessor
        loanRepaymentScheduleTransactionProcessor =
            this.transactionProcessorFactory.determineProcessor(
    ...
    changedTransactionDetail =
        loanRepaymentScheduleTransactionProcessor.handleTransaction(
            getDisbursementDate(), allNonContraTransactionsPostDisbursement,
   ...
   *
   * If "suppliedTransactionDate" is not passed Id, the transaction date is set
   * to the loans due date if the due date is lesser than todays date. If not,
   * the transaction date is set to todays date
   *
   * @param loanCharge
  ...
  handleChargeAppliedTransaction(final LoanCharge loanCharge,
    ...
    final LoanTransaction applyLoanChargeTransaction =
        LoanTransaction.accrueLoanCharge(
            this, getOffice(), chargeAmount, transactionDate, feeCharges,
            penaltyCharges, DateUtils.getLocalDateTimeOfTenant(), currentUser);

    Integer installmentNumber = null;
    final LoanChargePaidBy loanChargePaidBy = new LoanChargePaidBy(
        applyLoanChargeTransaction, loanCharge,
  ...
  private void handleChargePaidTransaction(
      final LoanCharge charge, final LoanTransaction chargesPayment,
      final LoanLifecycleStateMachine loanLifecycleStateMachine,
      final Integer installmentNumber) {
    chargesPayment.updateLoan(this);
    final LoanChargePaidBy loanChargePaidBy = new LoanChargePaidBy(
        chargesPayment, charge,
  ...
  private void validateLoanIsNotClosed(final LoanCharge loanCharge) {
    if (isClosed()) {
      ...
      throw new LoanChargeCannotBeAddedException("loanCharge", "loan.is.closed",
  ...
  private void validateLoanChargeIsNotWaived(final LoanCharge loanCharge) {
    if (loanCharge.isWaived()) {
      ...
      throw new LoanChargeCannotBeAddedException(
          "loanCharge", "loanCharge.is.waived", defaultUserMessage, getId(),
          loanCharge.name());
    }
  }
  ...
  private void validateChargeHasValidSpecifiedDateIfApplicable(
    ...
    if (loanCharge.isSpecifiedDueDate() &&
      ...
      throw new LoanChargeCannotBeAddedException(
          "loanCharge", "specified.due.date.outside.range", defaultUserMessage,
  ...
  public void removeLoanCharge(final LoanCharge loanCharge) {
    ...
    removeOrModifyTransactionAssociatedWithLoanChargeIfDueAtDisbursement(
        loanCharge);
    ...
    if (!loanCharge.isDueAtDisbursement() &&
      ...
      /****
       * TODO Vishwas Currently we do not allow removing a loan charge after a
       *loan is approved (hence there is no need to adjust any loan
       *transactions).
       *
  ...
  public Map<String, Object> updateLoanCharge(final LoanCharge loanCharge,
    ...
    if (charges().contains(loanCharge)) {
      final BigDecimal amount = calculateAmountPercentageAppliedTo(loanCharge);
      final Map<String, Object> loanChargeChanges =
          loanCharge.update(command, amount);
    ...
    if (!loanCharge.isDueAtDisbursement()) {
      /****
       * TODO Vishwas Currently we do not allow waiving updating loan charge
       *after a loan is approved (hence there is no need to adjust any loan
       *transactions).
       *
       * Consider removing this block of code or logically completing it for the
       *future by getting the list of affected Transactions
   ...
   */
  private BigDecimal
  calculateAmountPercentageAppliedTo(final LoanCharge loanCharge) {
    ...
    case PERCENT_OF_AMOUNT_AND_INTEREST:
      ...
      if (isMultiDisburmentLoan() && loanCharge.isDisbursementCharge()) {
        amount = getTotalAllTrancheDisbursementAmount().getAmount().add(
  ...
  public BigDecimal getTotalInterest() {
    return this.loanSummaryWrapper
        .calculateTotalInterestCharged(getRepaymentScheduleInstallments(),
                                       getCurrency())
        .getAmount();
  }

  private BigDecimal
  calculatePerInstallmentChargeAmount(final LoanCharge loanCharge) {
  ...
  private Money calculateInstallmentChargeAmount(
    ...
    amount =
        amount.plus(LoanCharge.percentageOf(percentOf.getAmount(), percentage));
    return amount;
  }

  public LoanTransaction
  waiveLoanCharge(final LoanCharge loanCharge,
                  final LoanLifecycleStateMachine loanLifecycleStateMachine,
    ...
    final Money amountWaived =
        loanCharge.waive(loanCurrency(), loanInstallmentNumber);
    ...
    if (isPeriodicAccrualAccountingEnabledOnLoanProduct()) {
      ...
      if (loanInstallmentNumber != null) {
        ...
        receivableCharge =
            accruedCharge.minus(loanCharge.getAmountPaid(getCurrency()));
      }
    ...
    final LoanTransaction waiveLoanChargeTransaction =
        LoanTransaction.waiveLoanCharge(
            this, getOffice(), amountWaived, transactionDate, feeChargesWaived,
            penaltyChargesWaived, unrecognizedIncome,
            DateUtils.getLocalDateTimeOfTenant(), currentUser);
    final LoanChargePaidBy loanChargePaidBy = new LoanChargePaidBy(
        waiveLoanChargeTransaction, loanCharge,
    ...
    if (!loanCharge.isDueAtDisbursement() &&
        loanCharge.isPaidOrPartiallyPaid(loanCurrency())) {
      /****
       * TODO Vishwas Currently we do not allow waiving fully paid loan charge
       *and waiving partially paid loan charges only waives the remaining
       *amount.
       *
       * Consider removing this block of code or logically completing it for the
       *future by getting the list of affected Transactions
    ...
    doPostLoanTransactionChecks(waiveLoanChargeTransaction.getTransactionDate(),
                                loanLifecycleStateMachine);
  ...
  public GroupLoanIndividualMonitoringAccount getGlim() { return glim; }

  public void setGlim(GroupLoanIndividualMonitoringAccount glim) {
    this.glim = glim;
  }

  public LoanProduct loanProduct() { return this.loanProduct; }

  public LoanProductRelatedDetail repaymentScheduleDetail() {
    return this.loanRepaymentScheduleDetail;
  }

  public void updateClient(final Client client) { this.client = client; }
  ...
  public void updateFund(final Fund fund) { this.fund = fund; }
  ...
  private void
  applyPeriodicAccruals(final Collection<LoanTransaction> accruals) {
    List<LoanRepaymentScheduleInstallment> installments =
        getRepaymentScheduleInstallments();
    for (LoanRepaymentScheduleInstallment installment : installments) {
    ...
    LoanRepaymentScheduleInstallment lastInstallment =
        getRepaymentScheduleInstallments().get(
            getRepaymentScheduleInstallments().size() - 1);
    for (LoanTransaction loanTransaction : accruals) {
      if (loanTransaction.getTransactionDate().isAfter(
  ...
  private void updateAccrualsForNonPeriodicAccruals(
    ...
    final Money interestApplied =
        Money.of(getCurrency(), this.summary.getTotalInterestCharged());
    for (LoanTransaction loanTransaction : accruals) {
  ...
  public void updateLoanScheduleDependentDerivedFields() {
    this.expectedMaturityDate =
        Date.from(determineExpectedMaturityDate()
  ...
  private void updateLoanSummaryDerivedFields() {
    ...
    if (isNotDisbursed()) {
      ...
      this.summary.updateSummary(
          loanCurrency(), principal, getRepaymentScheduleInstallments(),
  ...
  public Map<String, Object> loanApplicationModification(
    ...
    final Map<String, Object> actualChanges =
        this.loanRepaymentScheduleDetail.updateLoanApplicationAttributes(
    ...
    if (command.isChangeInStringParameterNamed(accountNoParamName,
      ...
      final String newValue =
          command.stringValueOfParameterNamed(accountNoParamName);
      actualChanges.put(accountNoParamName, newValue);
      this.accountNumber = StringUtils.defaultIfEmpty(newValue, null);
    }

    final String createSiAtDisbursementParameterName =
        "createStandingInstructionAtDisbursement";
    if (command.isChangeInBooleanParameterNamed(
      ...
      final Boolean valueAsInput = command.booleanObjectValueOfParameterNamed(
          createSiAtDisbursementParameterName);
    ...
    if (command.isChangeInLongParameterNamed(clientIdParamName, clientId)) {
      final Long newValue =
          command.longValueOfParameterNamed(clientIdParamName);
    ...
    if (command.isChangeInLongParameterNamed(productIdParamName,
      ...
      final Long newValue =
          command.longValueOfParameterNamed(productIdParamName);
    ...
    if (command.isChangeInBooleanParameterNamed(isFloatingInterestRateParamName,
                                                this.isFloatingInterestRate)) {
      final Boolean newValue = command.booleanObjectValueOfParameterNamed(
          isFloatingInterestRateParamName);
    ...
    if (command.isChangeInLongParameterNamed(loanOfficerIdParamName,
      ...
      final Long newValue =
          command.longValueOfParameterNamed(loanOfficerIdParamName);
    ...
    if (command.isChangeInLongParameterNamed(loanPurposeIdParamName,
      ...
      final Long newValue =
          command.longValueOfParameterNamed(loanPurposeIdParamName);
    ...
    if (command.isChangeInLongParameterNamed(
      ...
      final Long newValue =
          command.longValueOfParameterNamed(strategyIdParamName);
    ...
    if (command.isChangeInLocalDateParameterNamed(submittedOnDateParamName,
      ...
      final String valueAsInput =
          command.stringValueOfParameterNamed(submittedOnDateParamName);
      actualChanges.put(submittedOnDateParamName, valueAsInput);
      actualChanges.put("dateFormat", dateFormatAsInput);
      actualChanges.put("locale", localeAsInput);

      final LocalDate newValue =
          command.localDateValueOfParameterNamed(submittedOnDateParamName);
    ...
    final String repaymentsStartingFromDateParamName =
    ...
    if (command.isChangeInLocalDateParameterNamed(
      ...
      final LocalDate newValue = command.localDateValueOfParameterNamed(
          repaymentsStartingFromDateParamName);
      if (newValue != null) {
        this.expectedFirstRepaymentOnDate = Date.from(
    ...
    if (command.isChangeInLocalDateParameterNamed(
      ...
      final LocalDate newValue = command.localDateValueOfParameterNamed(
          interestChargedFromDateParamName);
      if (newValue != null) {
        this.interestChargedFromDate = Date.from(
            newValue.atStartOfDay(ZoneId.systemDefault()).toInstant());
      } else {
    ...
    if (getSubmittedOnDate().isAfter(DateUtils.getLocalDateOfTenant())) {
      final String errorMessage =
          "The date on which a loan is submitted cannot be in the future.";
      throw new InvalidLoanStateTransitionException(
          "submittal", "cannot.be.a.future.date", errorMessage,
          getSubmittedOnDate());
    }
    ...
    if (!(this.client == null)) {
      if (getSubmittedOnDate().isBefore(this.client.getActivationLocalDate())) {
        ...
        throw new InvalidLoanStateTransitionException(
            "submittal", "cannot.be.before.client.activation.date",
      ...
      if (getSubmittedOnDate().isBefore(this.group.getActivationLocalDate())) {
        ...
        throw new InvalidLoanStateTransitionException(
            "submittal", "cannot.be.before.group.activation.date", errorMessage,
            getSubmittedOnDate());
      }
    }
    ...
    if (getSubmittedOnDate().isAfter(getExpectedDisbursedOnLocalDate())) {
      ...
      throw new InvalidLoanStateTransitionException(
          "submittal", "cannot.be.after.expected.disbursement.date",
          errorMessage, getSubmittedOnDate(),
          getExpectedDisbursedOnLocalDate());
    }
    ...
    if (command.isChangeInIntegerParameterNamed(loanTermFrequencyParamName,
      ...
      final Integer newValue =
          command.integerValueOfParameterNamed(loanTermFrequencyParamName);
      actualChanges.put(externalIdParamName, newValue);
      this.termFrequency = newValue;
    }

    final String loanTermFrequencyTypeParamName = "loanTermFrequencyType";
    if (command.isChangeInIntegerParameterNamed(loanTermFrequencyTypeParamName,
                                                this.termPeriodFrequencyType)) {
      final Integer newValue =
          command.integerValueOfParameterNamed(loanTermFrequencyTypeParamName);
      final PeriodFrequencyType newTermPeriodFrequencyType =
          PeriodFrequencyType.fromInt(newValue);
    ...
    if (command.isChangeInBigDecimalParameterNamed(principalParamName,
      ...
      final BigDecimal newValue =
          command.bigDecimalValueOfParameterNamed(principalParamName);
    ...
    if (loanProduct.isMultiDisburseLoan()) {
      ...
      final JsonArray disbursementDataArray = command.arrayOfParameterNamed(
      ...
      if (disbursementDataArray.size() > loanProduct.maxTrancheCount()) {
        final String errorMessage =
            "Number of tranche shouldn't be greter than " +
    ...
    if (loanProduct.isMultiDisburseLoan() ||
      ...
      if (command.isChangeInBigDecimalParameterNamed(
        ...
        this.fixedEmiAmount = command.bigDecimalValueOfParameterNamed(
  ...
  private void updateOverdueScheduleInstallment(final LoanCharge loanCharge) {
    if (loanCharge.isOverdueInstallmentCharge() && loanCharge.isActive()) {
      ...
      if (overdueInstallmentCharge != null) {
        ...
        overdueInstallmentCharge.updateLoanRepaymentScheduleInstallment(
            installment);
      }
    }
  }
  ...
  private BigDecimal
  calculateOverdueAmountPercentageAppliedTo(final LoanCharge loanCharge,
                                            final int penaltyWaitPeriod) {
    LoanRepaymentScheduleInstallment installment =
  ...
  getDateFormatAndLocale(final JsonCommand jsonCommand) {
  ...
  parseDisbursementDetails(final JsonObject jsonObject, String dateFormat,
                           Locale locale) {
    Map<String, Object> returnObject = new HashMap<>();
    if (jsonObject.get(LoanApiConstants.disbursementDateParameterName) !=
            null &&
        jsonObject.get(LoanApiConstants.disbursementDateParameterName)
            .isJsonPrimitive()) {
      final JsonPrimitive primitive =
          jsonObject.get(LoanApiConstants.disbursementDateParameterName)
              .getAsJsonPrimitive();
      ...
      if (StringUtils.isNotBlank(valueAsString)) {
        LocalDate date = JsonParserHelper.convertFrom(
            valueAsString, LoanApiConstants.disbursementDateParameterName,
            dateFormat, locale);
        if (date != null) {
          returnObject.put(
    ...
    if (jsonObject.get(LoanApiConstants.disbursementPrincipalParameterName)
      ...
      BigDecimal principal =
          jsonObject
              ...
              .getAsBigDecimal();
      returnObject.put(LoanApiConstants.disbursementPrincipalParameterName,
                       principal);
    }
    ...
    if (jsonObject.has(LoanApiConstants.disbursementIdParameterName) &&
      ...
      Long id =
          jsonObject
              .getAsJsonPrimitive(LoanApiConstants.disbursementIdParameterName)
              .getAsLong();
  ...
  public void
  updateDisbursementDetails(final JsonCommand jsonCommand,
                            final Map<String, Object> actualChanges) {
    ...
    if (jsonCommand.parameterExists(LoanApiConstants.chargesParameterName)) {
      JsonArray chargesArray = jsonCommand.arrayOfParameterNamed(
    ...
    if (jsonCommand.parameterExists(
            LoanApiConstants.disbursementDataParameterName)) {
      final JsonArray disbursementDataArray = jsonCommand.arrayOfParameterNamed(
          LoanApiConstants.disbursementDataParameterName);
      if (disbursementDataArray != null && disbursementDataArray.size() > 0) {
        ...
        dateFormat =
            dateAndLocale.get(LoanApiConstants.dateFormatParameterName);
        if (dateAndLocale.containsKey(LoanApiConstants.localeParameterName)) {
          locale = JsonParserHelper.localeFromString(
        ...
        for (JsonElement jsonElement : disbursementDataArray) {
          ...
          Date expectedDisbursementDate = (Date)parsedDisbursementData.get(
              LoanApiConstants.disbursementDateParameterName);
          BigDecimal principal = (BigDecimal)parsedDisbursementData.get(
              LoanApiConstants.disbursementPrincipalParameterName);
          Long disbursementID = (Long)parsedDisbursementData.get(
              LoanApiConstants.disbursementIdParameterName);
          chargeIds = (String)parsedDisbursementData.get(
              LoanApiConstants.loanChargeIdParameterName);
          if (chargeIds != null) {
          ...
          createOrUpdateDisbursementDetails(disbursementID, actualChanges,
                                            expectedDisbursementDate, principal,
                                            disbursementList);
        }
        removeDisbursementAndAssociatedCharges(actualChanges, disbursementList,
                                               loanChargeIds, chargeIdLength,
                                               removeAllChages);
      }
    }
  }
  ...
  private void removeDisbursementAndAssociatedCharges(
      final Map<String, Object> actualChanges, List<Long> disbursementList,
  ...
  private void createOrUpdateDisbursementDetails(
      Long disbursementID, final Map<String, Object> actualChanges,
      Date expectedDisbursementDate, BigDecimal principal,
      List<Long> existingDisbursementList) {

    if (disbursementID != null) {
      LoanDisbursementDetails loanDisbursementDetail =
          fetchLoanDisbursementsById(disbursementID);
      ...
      if (loanDisbursementDetail.actualDisbursementDate() == null) {
        ...
        LoanDisbursementDetails disbursementDetails =
            new LoanDisbursementDetails(expectedDisbursementDate,
      ...
      for (LoanTrancheCharge trancheCharge : trancheCharges) {
        ...
        final LoanCharge loanCharge = LoanCharge.createNewWithoutLoan(
            ...
            LocalDate.ofInstant(expectedDisbursementDate.toInstant(),
  ...
  private CollateralData[] listOfLoanCollateralData(
    ...
    existingLoanCollateral = loanCollateralList.toArray(
        new CollateralData[loanCollateralList.size()]);
  ...
  private LoanChargeCommand[] getLoanCharges(
    ...
    existingLoanCharges =
        loanChargesList.toArray(new LoanChargeCommand[loanChargesList.size()]);
  ...
  public void loanApplicationSubmittal(
      final AppUser currentUser, final LoanScheduleModel loanSchedule,
    ...
    final LoanStatus statusEnum =
        lifecycleStateMachine.transition(LoanEvent.LOAN_CREATED, from);
    this.loanStatus = statusEnum.getValue();
    ...
    this.submittedOnDate =
        Date.from(submittedOn.atStartOfDay(ZoneId.systemDefault()).toInstant());
    this.submittedBy = currentUser;
    this.expectedDisbursementDate =
        Date.from(loanApplicationTerms.getExpectedDisbursementDate()
                      .atStartOfDay(ZoneId.systemDefault())
                      .toInstant());
    ...
    if (this.client != null && this.client.isActivatedAfter(submittedOn)) {
      final String errorMessage =
          "The date on which a loan is submitted cannot be earlier than client's activation date.";
      throw new InvalidLoanStateTransitionException(
          "submittal", "cannot.be.before.client.activation.date", errorMessage,
          submittedOn);
    }
  ...
  private LocalDate determineExpectedMaturityDate() {
    ...
    LocalDate maturityDate =
        installments.get(numberOfInstallments - 1).getDueDate();
  ...
  public Map<String, Object> loanApplicationRejection(
    ...
    if (!statusEnum.hasStateOf(LoanStatus.fromInt(this.loanStatus))) {
      ...
      final LocalDate rejectedOn =
          command.localDateValueOfParameterNamed("rejectedOnDate");

      final Locale locale = new Locale(command.locale());
      final DateTimeFormatter fmt =
          DateTimeFormatter.ofPattern(command.dateFormat()).withLocale(locale);

      this.rejectedOnDate = Date.from(
          rejectedOn.atStartOfDay(ZoneId.systemDefault()).toInstant());
      this.rejectedBy = currentUser;
      this.closedOnDate = Date.from(
          rejectedOn.atStartOfDay(ZoneId.systemDefault()).toInstant());
      ...
      if (rejectedOn.isBefore(getSubmittedOnDate())) {
        ...
        throw new InvalidLoanStateTransitionException(
            "reject", "cannot.be.before.submittal.date", errorMessage,
      ...
      if (rejectedOn.isAfter(DateUtils.getLocalDateOfTenant())) {
        final String errorMessage =
            "The date on which a loan is rejected cannot be in the future.";
        throw new InvalidLoanStateTransitionException(
            "reject", "cannot.be.a.future.date", errorMessage, rejectedOn);
      }
    } else {
      final String errorMessage =
          "Only the loan applications with status 'Submitted and pending approval' are allowed to be rejected.";
      throw new InvalidLoanStateTransitionException("reject", "cannot.reject",
                                                    errorMessage);
    }
  ...
  public Map<String, Object> loanApplicationWithdrawnByApplicant(
    ...
    if (!statusEnum.hasStateOf(LoanStatus.fromInt(this.loanStatus))) {
      ...
      LocalDate withdrawnOn =
          command.localDateValueOfParameterNamed("withdrawnOnDate");
      if (withdrawnOn == null) {
      ...
      this.withdrawnOnDate = Date.from(
          withdrawnOn.atStartOfDay(ZoneId.systemDefault()).toInstant());
      ...
      if (withdrawnOn.isBefore(getSubmittedOnDate())) {
        final String errorMessage =
            ...
            getSubmittedOnDate().toString();
        throw new InvalidLoanStateTransitionException(
            "withdraw", "cannot.be.before.submittal.date", errorMessage,
      ...
      final String errorMessage =
          "Only the loan applications with status 'Submitted and pending approval' are allowed to be withdrawn by applicant.";
      throw new InvalidLoanStateTransitionException(
  ...
  public Map<String, Object> loanApplicationApproval(
     ...
     * return invalidate state and below if condition should check for not equal
     * to invalidateState, instead of check new value is same as present value.
     */
    ...
    if (!newStatusEnum.hasStateOf(LoanStatus.fromInt(this.loanStatus))) {
      ...
      LocalDate approvedOn =
          command.localDateValueOfParameterNamed("approvedOnDate");
      String approvedOnDateChange =
          command.stringValueOfParameterNamed("approvedOnDate");
      if (approvedOn == null) {
      ...
      BigDecimal approvedLoanAmount = command.bigDecimalValueOfParameterNamed(
      ...
      if (approvedLoanAmount != null) {
        ...
        if (approvedLoanAmount.compareTo(this.proposedPrincipal) < 0) {
          ...
          /*
           * All the calculations are done based on the principal amount, so it
           * is necessary to set principal amount to approved amount
           */
          ...
          throw new InvalidLoanStateTransitionException(
              "approval", "amount.can't.be.greater.than.loan.amount.demanded",
      ...
      this.approvedOnDate = Date.from(
          approvedOn.atStartOfDay(ZoneId.systemDefault()).toInstant());
      this.approvedBy = currentUser;
      actualChanges.put("locale", command.locale());
      actualChanges.put("dateFormat", command.dateFormat());
      actualChanges.put("approvedOnDate", approvedOnDateChange);

      final LocalDate submittalDate = LocalDate.ofInstant(
          this.submittedOnDate.toInstant(), ZoneId.systemDefault());
      if (approvedOn.isBefore(submittalDate)) {
        ...
        throw new InvalidLoanStateTransitionException(
            "approval", "cannot.be.before.submittal.date", errorMessage,
            getApprovedOnDate(), submittalDate);
      }

      if (expecteddisbursementDate != null) {
        this.expectedDisbursementDate = Date.from(
            expecteddisbursementDate.atStartOfDay(ZoneId.systemDefault())
                .toInstant());
        ...
        if (expecteddisbursementDate.isBefore(approvedOn)) {
          final String errorMessage =
              ...
              approvedOn.toString();
          throw new InvalidLoanStateTransitionException(
              "expecteddisbursal", "should.be.on.or.after.approval.date",
              errorMessage, getApprovedOnDate(), expecteddisbursementDate);
        }
      }
      ...
      if (approvedOn.isAfter(DateUtils.getLocalDateOfTenant())) {
        ...
        throw new InvalidLoanStateTransitionException(
            "approval", "cannot.be.a.future.date", errorMessage,
            getApprovedOnDate());
      }

      if (this.loanOfficer != null) {
        final LoanOfficerAssignmentHistory loanOfficerAssignmentHistory =
            LoanOfficerAssignmentHistory.createNew(this, this.loanOfficer,
                                                   approvedOn);
  ...
  undoApproval(final LoanLifecycleStateMachine loanLifecycleStateMachine) {
  ...
  disburse(final AppUser currentUser, final JsonCommand command,
           final Map<String, Object> actualChanges,
           final ScheduleGeneratorDTO scheduleGeneratorDTO,
           final PaymentDetail paymentDetail) {

    final LoanStatus statusEnum = this.loanLifecycleStateMachine.transition(
    ...
    /**
     * Add an interest applied transaction of the interest is accrued upfront
     *(Up front accrual), no accounting or cash based accounting is selected
     **/
    ...
    if ((isNoneOrCashOrUpfrontAccrualAccountingEnabledOnLoanProduct() &&
      ...
      final LoanTransaction interestAppliedTransaction =
          LoanTransaction.accrueInterest(getOffice(), this, interestApplied,
                                         actualDisbursementDate, createdDate,
                                         currentUser);
      addLoanTransaction(interestAppliedTransaction);
    }
  ...
  public void regenerateScheduleOnDisbursement(
      final ScheduleGeneratorDTO scheduleGeneratorDTO,
      final boolean recalculateSchedule, final LocalDate actualDisbursementDate,
      BigDecimal emiAmount, final AppUser currentUser,
      LocalDate nextPossibleRepaymentDate, Date rescheduledRepaymentDate) {
    boolean isEmiAmountChanged = false;
    if ((this.loanProduct.isMultiDisburseLoan() ||
         this.loanProduct.canDefineInstallmentAmount()) &&
      ...
      if (this.loanProduct.isMultiDisburseLoan()) {
        ...
        Date effectiveDateFrom = Date.from(
            actualDisbursementDate.atStartOfDay(ZoneId.systemDefault())
                .toInstant());
        if (!isChangeEmiIfRepaymentDateSameAsDisbursementDateEnabled &&
            actualDisbursementDate.equals(nextPossibleRepaymentDate)) {
          effectiveDateFrom =
              Date.from(nextPossibleRepaymentDate.plusDays(1)
                            .atStartOfDay(ZoneId.systemDefault())
                            .toInstant());
        }
        LoanTermVariations loanVariationTerms = new LoanTermVariations(
            LoanTermVariationType.EMI_AMOUNT.getValue(), effectiveDateFrom,
            emiAmount, dateValue, isSpecificToInstallment, this,
    ...
    if (rescheduledRepaymentDate != null &&
      ...
      LoanTermVariations loanVariationTerms = new LoanTermVariations(
          ...
          Date.from(
              nextPossibleRepaymentDate.atStartOfDay(ZoneId.systemDefault())
                  .toInstant()),
          emiAmount, rescheduledRepaymentDate, isSpecificToInstallment, this,
          LoanStatus.ACTIVE.getValue());
      this.loanTermVariations.add(loanVariationTerms);
    }
  ...
  public boolean canDisburse(final LocalDate actualDisbursementDate) {
    ...
    if (LoanStatus.fromInt(this.loanStatus).isActive() &&
      ...
      if (Date.from(actualDisbursementDate.atStartOfDay(ZoneId.systemDefault())
                        .toInstant())
              .before(lastDusburseDate)) {
        final String errorMsg =
            "Loan can't be disbursed before " + lastDusburseDate;
        throw new LoanDisbursalException(
            errorMsg, "actualdisbursementdate.before.lastdusbursedate",
            lastDusburseDate,
            Date.from(
                actualDisbursementDate.atStartOfDay(ZoneId.systemDefault())
                    .toInstant()));
  ...
  public Money adjustDisburseAmount(final JsonCommand command,
    ...
    Money disburseAmount =
        this.loanRepaymentScheduleDetail.getPrincipal().zero();
    BigDecimal principalDisbursed = command.bigDecimalValueOfParameterNamed(
        LoanApiConstants.principalDisbursedParameterName);
    if (this.actualDisbursementDate == null) {
      this.actualDisbursementDate =
          Date.from(actualDisbursementDate.atStartOfDay(ZoneId.systemDefault())
                        .toInstant());
    }
    ...
    if (principalDisbursed == null) {
      ...
      if (!details.isEmpty()) {
        ...
        for (LoanDisbursementDetails disbursementDetails : details) {
          disbursementDetails.updateActualDisbursementDate(Date.from(
              actualDisbursementDate.atStartOfDay(ZoneId.systemDefault())
                  .toInstant()));
      ...
      if (this.loanProduct().isMultiDisburseLoan()) {
        ...
        if (totalAmount.compareTo(this.approvedPrincipal) > 0) {
          ...
          throw new LoanDisbursalException(
              errorMsg, "disburse.amount.must.be.less.than.approved.principal",
      ...
      if (!this.loanProduct().isMultiDisburseLoan() &&
        ...
        throw new LoanDisbursalException(
            errorMsg, "disburse.amount.must.be.less.than.approved.amount",
            principalDisbursed,
            this.loanRepaymentScheduleDetail.getPrincipal().getAmount());
      }
    }
  ...
  private Collection<LoanDisbursementDetails> fetchUndisbursedDetail() {
    ...
    for (LoanDisbursementDetails disbursementDetail :
      ...
      if (disbursementDetail.actualDisbursementDate() == null) {
        if (date == null ||
                    disbursementDetail.expectedDisbursementDate().compareTo(
                        date) == 0
                ? Boolean.TRUE
                : Boolean.FALSE) {
      ...
      for (LoanDisbursementDetails disbursementDetail :
        ...
        if (disbursementDetail.actualDisbursementDate() != null) {
          if (disbursementDetail.actualDisbursementDate().after(date) ||
                      disbursementDetail.actualDisbursementDate().compareTo(
                          date) == 0
                  ? Boolean.TRUE
                  : Boolean.FALSE) {
  ...
  private void removeDisbursementDetail() {
    Set<LoanDisbursementDetails> details =
        new HashSet<>(this.disbursementDetails);
   ...
   */
  public void
  regenerateRepaymentSchedule(final ScheduleGeneratorDTO scheduleGeneratorDTO,
                              final AppUser currentUser) {
  ...
  regenerateScheduleModel(final ScheduleGeneratorDTO scheduleGeneratorDTO) {
    ...
    if (loanApplicationTerms.isEqualAmortization()) {
      if (loanApplicationTerms.getInterestMethod().isDecliningBalnce()) {
        final LoanScheduleGenerator decliningLoanScheduleGenerator =
            scheduleGeneratorDTO.getLoanScheduleFactory().create(
    ...
    if (loanProduct.isLinkedToFloatingInterestRate()) {
      ...
      for (FloatingRatePeriodData periodData : applicableRates) {
        LoanTermVariationsData loanTermVariation = new LoanTermVariationsData(
            LoanEnumerations.loanvariationType(
                LoanTermVariationType.INTEREST_RATE),
            periodData.getFromDateAsLocalDate(), periodData.getInterestRate(),
            dateValue, isSpecificToInstallment);
        if (!interestRateStartDate.isBefore(
  ...
  private void handleDisbursementTransaction(
      final LocalDate disbursedOn, final LocalDateTime createdDate,
    ...
    for (final LoanCharge charge : charges()) {
      ...
      if ((charge.getCharge().getChargeTimeType().equals(
               ChargeTimeType.DISBURSEMENT.getValue()) &&
           disbursedOn.equals(LocalDate.ofInstant(
               actualDisbursementDate.toInstant(), ZoneId.systemDefault())) &&
           (actualDisbursementDate != null) && !charge.isWaived() &&
           !charge.isFullyPaid()) ||
          (charge.getCharge().getChargeTimeType().equals(
        ...
        if (totalFeeChargesDueAtDisbursement.isGreaterThanZero() &&
          ...
          final LoanChargePaidBy loanChargePaidBy = new LoanChargePaidBy(
              chargesPayment, charge, charge.amount(), installmentNumber);
          chargesPayment.getLoanChargesPaid().add(loanChargePaidBy);
          disbursentMoney = disbursentMoney.plus(charge.amount());
        }
      } else if (disbursedOn.equals(LocalDate.ofInstant(
                     this.actualDisbursementDate.toInstant(),
                     ZoneId.systemDefault()))) {
        /**
         * create a Charge applied transaction if Up front Accrual, None or Cash
         *based accounting is enabled
         **/
        if (isNoneOrCashOrUpfrontAccrualAccountingEnabledOnLoanProduct()) {
    ...
    if (disbursentMoney.isGreaterThanZero()) {
      ...
      chargesPayment.updateComponentsAndTotal(zero, zero, disbursentMoney,
                                              zero);
      chargesPayment.updateLoan(this);
      addLoanTransaction(chargesPayment);
      updateLoanOutstandingBalaces();
    ...
    if (getApprovedOnDate() != null &&
      ...
      throw new InvalidLoanStateTransitionException(
          "disbursal", "cannot.be.before.approval.date", errorMessage,
    ...
    if (getExpectedFirstRepaymentOnDate() != null &&
                ...
                Date.from(disbursedOn.atStartOfDay(ZoneId.systemDefault())
                              .toInstant())
                        .compareTo(this.actualDisbursementDate) == 0
            ? Boolean.TRUE
            : Boolean.FALSE) {
      final String errorMessage =
      ...
      throw new InvalidLoanStateTransitionException(
          "disbursal", "cannot.be.after.expected.first.repayment.date",
          errorMessage, disbursedOn, getExpectedFirstRepaymentOnDate());
    }

    validateActivityNotBeforeClientOrGroupTransferDate(LoanEvent.LOAN_DISBURSED,
                                                       disbursedOn);

    if (disbursedOn.isAfter(DateUtils.getLocalDateOfTenant())) {
      final String errorMessage =
          "The date on which a loan with identifier : " + this.accountNumber +
  ...
  public LoanTransaction handlePayDisbursementTransaction(
    ...
    chargesPayment.getLoanChargesPaid().add(loanChargePaidBy);
    final Money zero = Money.zero(getCurrency());
    chargesPayment.updateComponents(zero, zero, charge.getAmount(getCurrency()),
                                    zero);
    chargesPayment.updateLoan(this);
    addLoanTransaction(chargesPayment);
    updateLoanOutstandingBalaces();
  ...
  public Map<String, Object>
  undoDisbursal(final ScheduleGeneratorDTO scheduleGeneratorDTO,
                final List<Long> existingTransactionIds,
                final List<Long> existingReversedTransactionIds,
                AppUser currentUser) {
    ...
    final LoanStatus currentStatus = LoanStatus.fromInt(this.loanStatus);
    final LoanStatus statusEnum = this.loanLifecycleStateMachine.transition(
        LoanEvent.LOAN_DISBURSAL_UNDO, currentStatus);
    validateActivityNotBeforeClientOrGroupTransferDate(
    ...
    if (!statusEnum.hasStateOf(currentStatus)) {
      ...
      final boolean isScheduleRegenerateRequired =
          isRepaymentScheduleRegenerationRequiredForDisbursement(
              actualDisbursementDate);
      this.actualDisbursementDate = null;
      this.disbursedBy = null;
      boolean isDisbursedAmountChanged =
          approvedPrincipal.compareTo(
              this.loanRepaymentScheduleDetail.getPrincipal().getAmount()) == 0
              ? Boolean.FALSE
              : Boolean.TRUE;
      ...
      if (isScheduleRegenerateRequired || isDisbursedAmountChanged ||
          isEmiAmountChanged ||
        ...
        for (final LoanRepaymentScheduleInstallment period :
             getRepaymentScheduleInstallments()) {
  ...
  public ChangedTransactionDetail
  waiveInterest(final LoanTransaction waiveInterestTransaction,
                final LoanLifecycleStateMachine loanLifecycleStateMachine,
                final List<Long> existingTransactionIds,
                final List<Long> existingReversedTransactionIds,
                final ScheduleGeneratorDTO scheduleGeneratorDTO,
  ...
  @SuppressWarnings("null")
  public ChangedTransactionDetail
  makeRepayment(final LoanTransaction repaymentTransaction,
                final LoanLifecycleStateMachine loanLifecycleStateMachine,
                final List<Long> existingTransactionIds,
                final List<Long> existingReversedTransactionIds,
  ...
  public void
  makeChargePayment(final Long chargeId,
                    final LoanLifecycleStateMachine loanLifecycleStateMachine,
                    final List<Long> existingTransactionIds,
                    final List<Long> existingReversedTransactionIds,
    ...
    if (paymentTransaction.getTransactionDate().isAfter(
            DateUtils.getLocalDateOfTenant())) {
      final String errorMessage =
          "The date on which a loan charge paid cannot be in the future.";
      throw new InvalidLoanStateTransitionException(
          "charge.payment", "cannot.be.a.future.date", errorMessage,
          paymentTransaction.getTransactionDate());
    }
    existingTransactionIds.addAll(findExistingTransactionIds());
    existingReversedTransactionIds.addAll(findExistingReversedTransactionIds());
  ...
  public void
  makeRefund(final LoanTransaction loanTransaction,
             final LoanLifecycleStateMachine loanLifecycleStateMachine,
             final List<Long> existingTransactionIds,
             final List<Long> existingReversedTransactionIds,
    ...
    validateRepaymentDateIsOnNonWorkingDay(loanTransaction.getTransactionDate(),
                                           workingDays,
                                           allowTransactionsOnNonWorkingDay);
    ...
    if (status().isOverpaid()) {
      if (this.totalOverpaid.compareTo(
              loanTransaction.getAmount(getCurrency()).getAmount()) < 0) {
        final String errorMessage =
            "The refund amount must be less than or equal to overpaid amount ";
        throw new InvalidLoanStateTransitionException(
            "transaction", "is.exceeding.overpaid.amount", errorMessage,
            this.totalOverpaid,
            loanTransaction.getAmount(getCurrency()).getAmount());
      } else if (!isAfterLatRepayment(loanTransaction, getLoanTransactions())) {
        final String errorMessage =
            "Transfer funds is allowed only after last repayment date";
        throw new InvalidLoanStateTransitionException(
  ...
  private ChangedTransactionDetail handleRepaymentOrRecoveryOrWaiverTransaction(
    ...
    if (loanTransaction.isRecoveryRepayment() &&
        loanTransaction.getAmount(loanCurrency())
                .getAmount()
                .compareTo(getSummary().getTotalWrittenOff()) > 0) {
      final String errorMessage =
          "The transaction amount cannot greater than the remaining written off amount.";
      throw new InvalidLoanStateTransitionException(
          "transaction", "cannot.be.greater.than.total.written.off",
          errorMessage);
    }
    ...
    if (loanTransaction.isNotRepayment() && loanTransaction.isNotWaiver() &&
      ...
      final String errorMessage =
          "A transaction of type repayment or recovery repayment or waiver was expected but not received.";
      throw new InvalidLoanTransactionTypeException(
          "transaction", "is.not.a.repayment.or.waiver.or.recovery.transaction",
          errorMessage);
    }
    ...
    if (loanTransactionDate.isBefore(getDisbursementDate())) {
      final String errorMessage =
          ...
          getApprovedOnDate().toString();
      throw new InvalidLoanStateTransitionException(
          "transaction", "cannot.be.before.disbursement.date", errorMessage,
    ...
    if (loanTransactionDate.isAfter(DateUtils.getLocalDateOfTenant())) {
      ...
      throw new InvalidLoanStateTransitionException(
          "transaction", "cannot.be.a.future.date", errorMessage,
          loanTransactionDate);
    }

    if (loanTransaction.isInterestWaiver()) {
      Money totalInterestOutstandingOnLoan =
          getTotalInterestOutstandingOnLoan();
      if (adjustedTransaction != null) {
        totalInterestOutstandingOnLoan = totalInterestOutstandingOnLoan.plus(
            adjustedTransaction.getAmount(loanCurrency()));
      }
      if (loanTransaction.getAmount(loanCurrency())
              .isGreaterThan(totalInterestOutstandingOnLoan)) {
        final String errorMessage =
            "The amount of interest to waive cannot be greater than total interest outstanding on loan.";
        throw new InvalidLoanStateTransitionException(
            "waive.interest", "amount.exceeds.total.outstanding.interest",
            errorMessage, loanTransaction.getAmount(loanCurrency()),
    ...
    if (this.loanProduct.isMultiDisburseLoan() && adjustedTransaction == null) {
      ...
      if (totalDisbursed.compareTo(this.summary.getTotalPrincipalRepaid()) <
          0) {
        final String errorMessage =
    ...
    final LoanRepaymentScheduleInstallment currentInstallment =
        fetchLoanRepaymentScheduleInstallment(
    ...
    if (isTransactionChronologicallyLatest && adjustedTransaction == null &&
        ...
        !isForeclosure()) {
      loanRepaymentScheduleTransactionProcessor.handleTransaction(
          loanTransaction, getCurrency(), getRepaymentScheduleInstallments(),
          charges());
      reprocess = false;
      if (this.repaymentScheduleDetail().isInterestRecalculationEnabled()) {
        if (currentInstallment == null ||
          ...
          if (nextInstallment != null &&
              nextInstallment.getTotalPaidInAdvance(getCurrency())
                  .isGreaterThanZero()) {
            reprocess = true;
          }
        }
    ...
    if (reprocess) {
       ...
       * the transactions. for this need to save the reversed transactions first
  ...
  private List<LoanTransaction> retreiveListOfIncomePostingTransactions() {
    ...
    final LoanTransactionComparator transactionComparator =
        new LoanTransactionComparator();
  ...
  retreiveListOfTransactionsPostDisbursementExcludeAccruals() {
    ...
    for (final LoanTransaction transaction : this.loanTransactions) {
      if (transaction.isNotReversed() &&
          !(transaction.isDisbursement() || transaction.isAccrual() ||
  ...
  private void doPostLoanTransactionChecks(final LocalDate transactionDate,
                                           final LoanLifecycleStateMachine
                                               loanLifecycleStateMachine) {
  ...
  private void handleLoanRepaymentInFull(final LocalDate transactionDate,
    ...
    for (final LoanCharge loanCharge : this.charges) {
      if (loanCharge.isActive() &&
          loanCharge.amount().compareTo(BigDecimal.ZERO) > 0 &&
    ...
    if (isAllChargesPaid) {
      ...
      this.closedOnDate = Date.from(
          transactionDate.atStartOfDay(ZoneId.systemDefault()).toInstant());
      this.actualMaturityDate = Date.from(
          transactionDate.atStartOfDay(ZoneId.systemDefault()).toInstant());
  ...
  private void processIncomeAccrualTransactionOnLoanClosure() {
    if (this.loanInterestRecalculationDetails != null &&
      ...
      LocalDate closedLocalDate =
          LocalDate.ofInstant(closedDate.toInstant(), ZoneId.systemDefault());
      reverseTransactionsOnOrAfter(retreiveListOfIncomePostingTransactions(),
                                   closedDate);
      reverseTransactionsOnOrAfter(retreiveListOfAccrualTransactions(),
                                   closedDate);
      HashMap<String, BigDecimal> cumulativeIncomeFromInstallments =
          new HashMap<>();
      determineCumulativeIncomeFromInstallments(
          cumulativeIncomeFromInstallments);
      HashMap<String, BigDecimal> cumulativeIncomeFromIncomePosting =
          new HashMap<>();
      ...
      BigDecimal interestToPost =
          cumulativeIncomeFromInstallments.get("interest")
              .subtract(cumulativeIncomeFromIncomePosting.get("interest"));
      BigDecimal feeToPost =
          cumulativeIncomeFromInstallments.get("fee").subtract(
              cumulativeIncomeFromIncomePosting.get("fee"));
      BigDecimal penaltyToPost =
          cumulativeIncomeFromInstallments.get("penalty").subtract(
              cumulativeIncomeFromIncomePosting.get("penalty"));
      BigDecimal amountToPost =
          interestToPost.add(feeToPost).add(penaltyToPost);
      LoanTransaction finalIncomeTransaction = LoanTransaction.incomePosting(
          this, this.getOffice(), closedDate, amountToPost, interestToPost,
      ...
      if (isPeriodicAccrualAccountingEnabledOnLoanProduct()) {
        ...
        if (updatedAccrualTransactions != null &&
          ...
          lastAccruedDate = updatedAccrualTransactions
        ...
        LoanTransaction finalAccrual = LoanTransaction.accrueTransaction(
            this, this.getOffice(), closedLocalDate, amountToPost,
  ...
  private void determineCumulativeIncomeFromInstallments(
    ...
    for (LoanRepaymentScheduleInstallment installment : installments) {
      ...
      penalty = penalty.add(
          installment.getPenaltyChargesCharged(getCurrency()).getAmount());
    }
  ...
  private void determineCumulativeIncomeDetails(
    ...
    for (LoanTransaction transaction : transactions) {
      interest = interest.add(
          transaction.getInterestPortion(getCurrency()).getAmount());
      fee =
          fee.add(transaction.getFeeChargesPortion(getCurrency()).getAmount());
      penalty = penalty.add(
          transaction.getPenaltyChargesPortion(getCurrency()).getAmount());
    }
  ...
  private boolean isChronologicallyLatestRepaymentOrWaiver(
    ...
    for (final LoanTransaction previousTransaction : loanTransactions) {
      if (!previousTransaction.isDisbursement() &&
        ...
        if (currentTransactionDate.isBefore(
                previousTransaction.getTransactionDate()) ||
            currentTransactionDate.isEqual(
                previousTransaction.getTransactionDate())) {
  ...
  private boolean
  isAfterLatRepayment(final LoanTransaction loanTransaction,
                      final List<LoanTransaction> loanTransactions) {
  ...
  public LoanTransaction
  deriveDefaultInterestWaiverTransaction(final LocalDateTime createdDate,
                                         final AppUser currentUser) {
    ...
    if (totalInterestOutstanding.isGreaterThanZero()) {
      ...
      for (final LoanRepaymentScheduleInstallment scheduledRepayment :
        ...
        if (scheduledRepayment.isOverdueOn(
                LocalDate.now(ZoneId.systemDefault())) &&
    ...
    return LoanTransaction.waiver(getOffice(), this, possibleInterestToWaive,
                                  transactionDate, possibleInterestToWaive,
                                  possibleInterestToWaive.zero(), createdDate,
                                  currentUser);
  }
  ...
  public ChangedTransactionDetail adjustExistingTransaction(
    ...
    if (transactionForAdjustment.isNotRepayment() &&
      ...
      final String errorMessage =
          "Only transactions of type repayment or waiver can be adjusted.";
      throw new InvalidLoanTransactionTypeException(
          "transaction",
  ...
  public ChangedTransactionDetail
  undoWrittenOff(final List<Long> existingTransactionIds,
                 final List<Long> existingReversedTransactionIds,
                 final ScheduleGeneratorDTO scheduleGeneratorDTO,
                 final AppUser currentUser) {
  ...
  private Money calculateTotalOverpayment() {
    ...
    for (final LoanRepaymentScheduleInstallment scheduledRepayment :
      ...
      cumulativeTotalPaidOnInstallments =
          cumulativeTotalPaidOnInstallments
              .plus(scheduledRepayment.getPrincipalCompleted(currency).plus(
      ...
      cumulativeTotalWaivedOnInstallments =
          cumulativeTotalWaivedOnInstallments.plus(
              scheduledRepayment.getInterestWaived(currency));
    }
  ...
  public ChangedTransactionDetail
  closeAsWrittenOff(final JsonCommand command,
                    final LoanLifecycleStateMachine loanLifecycleStateMachine,
                    final Map<String, Object> changes,
                    final List<Long> existingTransactionIds,
                    final List<Long> existingReversedTransactionIds,
    ...
    ChangedTransactionDetail changedTransactionDetail = closeDisbursements(
        scheduleGeneratorDTO, loanRepaymentScheduleTransactionProcessor,
        currentUser);
    ...
    if (!statusEnum.hasStateOf(LoanStatus.fromInt(this.loanStatus))) {
      ...
      existingTransactionIds.addAll(findExistingTransactionIds());
      existingReversedTransactionIds.addAll(
      ...
      this.closedOnDate =
          Date.from(writtenOffOnLocalDate.atStartOfDay(ZoneId.systemDefault())
                        .toInstant());
      this.writtenOffOnDate =
          Date.from(writtenOffOnLocalDate.atStartOfDay(ZoneId.systemDefault())
                        .toInstant());
      this.closedBy = currentUser;
      ...
      if (writtenOffOnLocalDate.isBefore(getDisbursementDate())) {
        ...
        throw new InvalidLoanStateTransitionException(
            "writeoff", "cannot.be.before.submittal.date", errorMessage,
      ...
      if (writtenOffOnLocalDate.isAfter(DateUtils.getLocalDateOfTenant())) {
        ...
        throw new InvalidLoanStateTransitionException(
            "writeoff", "cannot.be.a.future.date", errorMessage,
            writtenOffOnLocalDate);
      }

      LocalDateTime createdDate = DateUtils.getLocalDateTimeOfTenant();
      loanTransaction =
          LoanTransaction.writeoff(this, getOffice(), writtenOffOnLocalDate,
                                   txnExternalId, createdDate, currentUser);
      ...
      if (lastTransactionDate.isAfter(writtenOffOnLocalDate)) {
        ...
        throw new InvalidLoanStateTransitionException(
            "writeoff", "must.occur.on.or.after.other.transaction.dates",
  ...
  closeDisbursements(final ScheduleGeneratorDTO scheduleGeneratorDTO,
                     final LoanRepaymentScheduleTransactionProcessor
                         loanRepaymentScheduleTransactionProcessor,
                     final AppUser currentUser) {
    ChangedTransactionDetail changedTransactionDetail = null;
  ...
  private LoanTransaction findlatestTransaction() {
    ...
    for (LoanTransaction loanTransaction : this.loanTransactions) {
      if (!loanTransaction.isReversed() &&
          (transaction == null || transaction.getTransactionDate().isBefore(
                                      loanTransaction.getTransactionDate()))) {
  ...
  public ChangedTransactionDetail
  close(final JsonCommand command,
        final LoanLifecycleStateMachine loanLifecycleStateMachine,
        final Map<String, Object> changes,
        final List<Long> existingTransactionIds,
        final List<Long> existingReversedTransactionIds,
    ...
    validateActivityNotBeforeClientOrGroupTransferDate(LoanEvent.REPAID_IN_FULL,
                                                       closureDate);
    if (closureDate.isBefore(getDisbursementDate())) {
      final String errorMessage =
          ...
          getDisbursementDate().toString();
      throw new InvalidLoanStateTransitionException(
          "close", "cannot.be.before.submittal.date", errorMessage, closureDate,
          getDisbursementDate());
    }

    if (closureDate.isAfter(DateUtils.getLocalDateOfTenant())) {
      final String errorMessage =
          "The date on which a loan is closed cannot be in the future.";
      throw new InvalidLoanStateTransitionException(
    ...
    if (isOpen()) {
      ...
      if (totalOutstanding.isGreaterThanZero() &&
        ...
        loanTransaction = LoanTransaction.writeoff(
            this, getOffice(), closureDate, txnExternalId,
            DateUtils.getLocalDateTimeOfTenant(), currentUser);
        ...
        if (!isLastTransaction) {
          ...
          throw new InvalidLoanStateTransitionException(
              "close.loan", "must.occur.on.or.after.latest.transaction.date",
        ...
        final String errorMessage =
            "A loan with money outstanding cannot be closed";
        throw new InvalidLoanStateTransitionException(
            "close", "loan.has.money.outstanding", errorMessage,
    ...
    if (isOverPaid()) {
      ...
      if (totalLoanOverpayment.isGreaterThanZero() &&
        ...
        throw new InvalidLoanStateTransitionException(
            "close", "loan.is.overpaid", errorMessage,
  ...
  public void closeAsMarkedForReschedule(final JsonCommand command,
    ...
    this.closedOnDate = Date.from(
        rescheduledOn.atStartOfDay(ZoneId.systemDefault()).toInstant());
    this.rescheduledOnDate = Date.from(
        rescheduledOn.atStartOfDay(ZoneId.systemDefault()).toInstant());
    changes.put("closedOnDate",
                command.stringValueOfParameterNamed("transactionDate"));
    ...
    final LocalDate rescheduledOnLocalDate = LocalDate.ofInstant(
        this.rescheduledOnDate.toInstant(), ZoneId.systemDefault());
    if (rescheduledOnLocalDate.isBefore(getDisbursementDate())) {
      final String errorMessage =
          ...
          getDisbursementDate().toString();
      throw new InvalidLoanStateTransitionException(
          "close.reschedule", "cannot.be.before.submittal.date", errorMessage,
    ...
    if (rescheduledOnLocalDate.isAfter(DateUtils.getLocalDateOfTenant())) {
      ...
      throw new InvalidLoanStateTransitionException(
          "close.reschedule", "cannot.be.a.future.date", errorMessage,
  ...
  public LoanStatus status() { return LoanStatus.fromInt(this.loanStatus); }
  ...
  private boolean isNotDisbursed() { return !isDisbursed(); }
  ...
  public boolean isClosed() { return status().isClosed() || isCancelled(); }

  private boolean isClosedObligationsMet() {
    return status().isClosedObligationsMet();
  }

  public boolean isClosedWrittenOff() { return status().isClosedWrittenOff(); }
  ...
  private boolean isCancelled() { return isRejected() || isWithdrawn(); }

  private boolean isWithdrawn() { return status().isWithdrawnByClient(); }

  private boolean isRejected() { return status().isRejected(); }

  public boolean isOpen() { return status().isActive(); }
  ...
  public boolean isSubmittedOnDateAfter(final LocalDate compareDate) {
    return this.submittedOnDate == null
        ? false
        : LocalDate
              .ofInstant(this.submittedOnDate.toInstant(),
                         ZoneId.systemDefault())
              .isAfter(compareDate);
  }
  ...
  public LocalDate getSubmittedOnDate() {
    return ObjectUtils.defaultIfNull(
        LocalDate.ofInstant(this.submittedOnDate.toInstant(),
                            ZoneId.systemDefault()),
        null);
  }
  ...
  public LocalDate getApprovedOnDate() {
    ...
    if (this.approvedOnDate != null) {
      date = LocalDate.ofInstant(this.approvedOnDate.toInstant(),
                                 ZoneId.systemDefault());
    }
    return date;
  }
  ...
  public LocalDate getExpectedDisbursedOnLocalDate() {
    ...
    if (this.expectedDisbursementDate != null) {
      expectedDisbursementDate = LocalDate.ofInstant(
          this.expectedDisbursementDate.toInstant(), ZoneId.systemDefault());
    }
  ...
  public LocalDate getDisbursementDate() {
    ...
    if (this.actualDisbursementDate != null) {
      disbursementDate = LocalDate.ofInstant(
          this.actualDisbursementDate.toInstant(), ZoneId.systemDefault());
    }
  ...
  public LocalDate getWrittenOffDate() {
    ...
    if (this.writtenOffOnDate != null) {
      writtenOffDate = LocalDate.ofInstant(this.writtenOffOnDate.toInstant(),
                                           ZoneId.systemDefault());
    }
  ...
  public LocalDate getExpectedDisbursedOnLocalDateForTemplate() {
    ...
    if (!details.isEmpty()) {
      for (LoanDisbursementDetails disbursementDetails : details) {
        expectedDisbursementDate = LocalDate.ofInstant(
            disbursementDetails.expectedDisbursementDate().toInstant(),
            ZoneId.systemDefault());
      }
    }
    return expectedDisbursementDate;
  ...
  public LocalDate getExpectedFirstRepaymentOnDate() {
    ...
    if (this.expectedFirstRepaymentOnDate != null) {
      firstRepaymentDate =
          LocalDate.ofInstant(this.expectedFirstRepaymentOnDate.toInstant(),
                              ZoneId.systemDefault());
    }
  ...
  private boolean isActualDisbursedOnDateEarlierOrLaterThanExpected(
    ...
    if (this.loanProduct.isMultiDisburseLoan()) {
      LoanDisbursementDetails details = fetchLastDisburseDetail();
      if (details != null && details.expectedDisbursementDate().compareTo(
                                 details.actualDisbursementDate()) == 0
              ? Boolean.FALSE
              : Boolean.TRUE) {
    ...
    return !LocalDate
                .ofInstant(this.expectedDisbursementDate.toInstant(),
                           ZoneId.systemDefault())
  ...
  private boolean isRepaymentScheduleRegenerationRequiredForDisbursement(
      final LocalDate actualDisbursementDate) {
    return isActualDisbursedOnDateEarlierOrLaterThanExpected(
        actualDisbursementDate);
  }
  ...
  public Money getTotalRecoveredPayments() {
    ...
    for (final LoanTransaction recoveredPayment : this.loanTransactions) {
      if (recoveredPayment.isRecoveryRepayment()) {
        cumulativePaid =
            cumulativePaid.plus(recoveredPayment.getAmount(loanCurrency()));
      }
    }
    return cumulativePaid;
  }
  ...
  private Money getTotalInterestOutstandingOnLoan() {
    ...
    for (final LoanRepaymentScheduleInstallment scheduledRepayment :
         installments) {
      cumulativeInterest = cumulativeInterest.plus(
          scheduledRepayment.getInterestOutstanding(loanCurrency()));
    }
  ...
  private Money getTotalInterestOverdueOnLoan() {
    Money cumulativeInterestOverdue = Money.zero(
        this.loanRepaymentScheduleDetail.getPrincipal().getCurrency());
    List<LoanRepaymentScheduleInstallment> installments =
        getRepaymentScheduleInstallments();
    for (final LoanRepaymentScheduleInstallment scheduledRepayment :
         installments) {
      ...
      if (scheduledRepayment.isOverdueOn(
              LocalDate.now(ZoneId.systemDefault()))) {
        cumulativeInterestOverdue =
            cumulativeInterestOverdue.plus(interestOutstandingForPeriod);
      }
    }
  ...
  public boolean hasLoanOfficer(final Staff fromLoanOfficer) {
    ...
    if (this.loanOfficer != null) {
      matchesCurrentLoanOfficer =
          this.loanOfficer.identifiedBy(fromLoanOfficer);
    } else {
  ...
  public LocalDate getInterestChargedFromDate() {
    ...
    if (this.interestChargedFromDate != null) {
      interestChargedFrom = LocalDate.ofInstant(
          this.interestChargedFromDate.toInstant(), ZoneId.systemDefault());
    }
  ...
  public void reassignLoanOfficer(final Staff newLoanOfficer,
    ...
    if (isSubmittedOnDateAfter(assignmentDate)) {
      ...
      throw new LoanOfficerAssignmentDateException(
          "cannot.be.before.loan.submittal.date", errorMessage, assignmentDate,
          getSubmittedOnDate());
      ...
      final String errorMessage =
          "The Loan Officer assignment date (" + assignmentDate +
          ") cannot be before previous Loan Officer unassigned date (" +
          lastAssignmentRecord.getEndDate() + ").";

      throw new LoanOfficerAssignmentDateException(
          "cannot.be.before.previous.unassignement.date", errorMessage,
  ...
  private void
  validateUnassignDate(final LoanOfficerAssignmentHistory latestHistoryRecord,
    ...
    if (latestHistoryRecord.getStartDate().isAfter(unassignDate)) {
      ...
      throw new LoanOfficerUnassignmentDateException(
          ...
          getLoanOfficer().getId(), latestHistoryRecord.getStartDate(),
          unassignDate);
  ...
  private LoanOfficerAssignmentHistory findLatestIncompleteHistoryRecord() {
    ...
    for (final LoanOfficerAssignmentHistory historyRecord :
         this.loanOfficerHistory) {
  ...
  findLastAssignmentHistoryRecord(final Staff newLoanOfficer) {
    ...
    for (final LoanOfficerAssignmentHistory historyRecord :
      ...
      if (lastAssignmentRecordLatestEndDate == null) {
        lastAssignmentRecordLatestEndDate = historyRecord;
      } else if (historyRecord.isEndDateAfter(
                     lastAssignmentRecordLatestEndDate.getEndDate()) &&
                 !historyRecord.isSameLoanOfficer(newLoanOfficer)) {
        lastAssignmentRecordLatestEndDate = historyRecord;
      }
    }
  ...
  public Long productId() { return this.loanProduct.getId(); }

  public Staff getLoanOfficer() { return this.loanOfficer; }

  public LoanSummary getSummary() { return this.summary; }

  public Set<LoanCollateral> getCollateral() { return this.collateral; }

  public BigDecimal getProposedPrincipal() { return this.proposedPrincipal; }

  public Map<String, Object>
  deriveAccountingBridgeData(final CurrencyData currencyData,
                             final List<Long> existingTransactionIds,
                             final List<Long> existingReversedTransactionIds,
    ...
    for (final LoanTransaction transaction : this.loanTransactions) {
      if (transaction.isNotReversed() &&
        ...
        if (transaction.isAccrual()) {
          receivableInterest = receivableInterest.plus(
              transaction.getInterestPortion(getCurrency()));
        } else if (transaction.isRepayment() ||
                   transaction.isInterestWaiver()) {
          receivableInterest = receivableInterest.minus(
              transaction.getInterestPortion(getCurrency()));
        }
      }
       ...
       * transaction.isAccrual()) { final String errorMessage = "The date on
       * which a loan is interest waived cannot be in after accrual
       * transactions." ; throw new InvalidLoanStateTransitionException("waive",
  ...
  public void
  setHelpers(final LoanLifecycleStateMachine loanLifecycleStateMachine,
  ...
  public boolean isSyncDisbursementWithMeeting() {
    return this.syncDisbursementWithMeeting == null
        ? false
        : this.syncDisbursementWithMeeting;
  }

  public Date getClosedOnDate() { return this.closedOnDate; }
  ...
  public void updateLoanRepaymentScheduleDates(
    ...
    /*
     * meetingStartDate is used as seedDate Capture the seedDate from user and
     * use the seedDate as meetingStart date
     */
    ...
    final Integer loanRepaymentInterval =
        this.loanRepaymentScheduleDetail.getRepayEvery();
      ...
      if (oldDueDate.isEqual(presentMeetingDate) ||
        ...
        if (isFirstTime) {
          ...
          newRepaymentDate = CalendarUtils.getNewRepaymentMeetingDate(
              recuringRule, tmpFromDate, tmpFromDate.plusDays(1),
              loanRepaymentInterval, frequency, workingDays,
              isSkipRepaymentonfirstdayofmonth, numberofDays);
        }

        if (isHolidayEnabled) {
          newRepaymentDate = HolidayUtil.getRepaymentRescheduleDateToIfHoliday(
    ...
    if (latestRepaymentDate != null) {
      this.expectedMaturityDate = Date.from(
          latestRepaymentDate.atStartOfDay(ZoneId.systemDefault()).toInstant());
    }
  }
  ...
  public void updateLoanRepaymentScheduleDates(
    ...
    for (final LoanRepaymentScheduleInstallment
      ...
      if (oldDueDate.isAfter(seedDate) &&
        ...
        newRepaymentDate = CalendarUtils.getNewRepaymentMeetingDate(
            recuringRule, seedDate, oldDueDate, loanRepaymentInterval,
        ...
        if (newRepaymentDate.isAfter(maxDateLimitForNewRepayment)) {
          newRepaymentDate = CalendarUtils.getNextRepaymentMeetingDate(
              recuringRule, seedDate, tmpFromDate, loanRepaymentInterval,
              frequency, workingDays, isSkipRepaymentonfirstdayofmonth,
              numberofDays);
        }
  ...
  private LocalDate
  getMaxDateLimitForNewRepayment(final PeriodFrequencyType periodFrequencyType,
                                 final Integer loanRepaymentInterval,
  ...
  public void
  applyHolidayToRepaymentScheduleDates(final Holiday holiday,
                                       final LoanUtilService loanUtilService) {
    final DefaultScheduledDateGenerator scheduledDateGenerator =
        new DefaultScheduledDateGenerator();
    ScheduleGeneratorDTO scheduleGeneratorDTO =
        loanUtilService.buildScheduleGeneratorDTO(
            this, holiday.getFromDateLocalDate());
    ...
    if (holiday.getReScheduleType().isResheduleToNextRepaymentDate()) {
      ...
      for (final LoanRepaymentScheduleInstallment
           ...
           this.getRepaymentScheduleInstallments()) {
        if (rescheduleToDate.isEqual(
                loanRepaymentScheduleInstallment.getDueDate())) {
          adjustedRescheduleToDate = rescheduleToDate;
        } else if (rescheduleToDate.isAfter(
                       loanRepaymentScheduleInstallment.getDueDate())) {
          if (this.getLoanProduct().getFirstSemiDate() != null &&
              this.getLoanProduct().getSecondSemiDate() != null) {
            if (rescheduleToDate.isBefore(
                    loanRepaymentScheduleInstallment.getDueDate().plusDays(
                        15))) {
    ...
    // ArrayList<>(); for (LoanTermVariationsData loanTermVariation :
    ...
    // scheduledDateGenerator.generateNextRepaymentDate(rescheduleFromDate,
    // loanApplicationTerms,
    // false);
    // if (loanTermVariation.getTermApplicableFrom().equals(applicableDate)) {
    // LocalDate adjustedDate =
    // scheduledDateGenerator.generateNextRepaymentDate(adjustedApplicableDate,
    ...
    for (final LoanRepaymentScheduleInstallment
      ...
      if (oldDueDate.equals(holiday.getFromDateLocalDate()) ||
        ...
        adjustedRescheduleToDate =
            scheduledDateGenerator.generateNextRepaymentDate(
                adjustedRescheduleToDate, loanApplicationTerms, false);

        // final LocalDate newRepaymentDate =
        // holiday.getRepaymentsRescheduledToLocalDate();
        loanRepaymentScheduleInstallment.updateDueDate(
            adjustedRescheduleToDate);
      }
  ...
  private void validateDisbursementDateIsOnNonWorkingDay(
    ...
    if (!allowTransactionsOnNonWorkingDay) {
      if (!WorkingDaysUtil.isWorkingDay(workingDays, getDisbursementDate())) {
        final String errorMessage =
            "Expected disbursement date cannot be on a non working day";
  ...
  private void
  validateDisbursementDateIsOnHoliday(final boolean allowTransactionsOnHoliday,
                                      final List<Holiday> holidays) {
    if (!allowTransactionsOnHoliday) {
      if (HolidayUtil.isHoliday(getDisbursementDate(), holidays)) {
        final String errorMessage =
            "Expected disbursement date cannot be on a holiday";
        throw new LoanApplicationDateException(
  ...
  public void
  validateRepaymentDateIsOnHoliday(final LocalDate repaymentDate,
                                   final boolean allowTransactionsOnHoliday,
                                   final List<Holiday> holidays) {
    if (!allowTransactionsOnHoliday) {
      if (HolidayUtil.isHoliday(repaymentDate, holidays)) {
        final String errorMessage = "Repayment date cannot be on a holiday";
        throw new LoanApplicationDateException("repayment.date.on.holiday",
  ...
  public void updateGroup(final Group newGroup) { this.group = newGroup; }

  public Integer getCurrentLoanCounter() { return this.loanCounter; }
  ...
  public void
  updateLoanProductLoanCounter(final Integer newLoanProductLoanCounter) {
  ...
  public Integer getTermFrequency() { return this.termFrequency; }
  ...
  public void validateExpectedDisbursementForHolidayAndNonWorkingDay(
      final WorkingDays workingDays, final boolean allowTransactionsOnHoliday,
      final List<Holiday> holidays,
      final boolean allowTransactionsOnNonWorkingDay) {
    // validate if disbursement date is a holiday or a non-working day
    validateDisbursementDateIsOnNonWorkingDay(workingDays,
  ...
  private void validateActivityNotBeforeClientOrGroupTransferDate(
    ...
    if (this.client != null &&
      ...
      final LocalDate clientOfficeJoiningDate =
          this.client.getOfficeJoiningLocalDate();
      if (activityDate.isBefore(clientOfficeJoiningDate)) {
        ...
        case LOAN_DISBURSED:
          errorMessage =
              "The date on which a loan is disbursed cannot be earlier than client's transfer date to this office";
          action = "disbursal";
          postfix = "cannot.be.before.client.transfer.date";
          break;
        ...
        case LOAN_REPAYMENT_OR_WAIVER:
          errorMessage =
              "The date on which a repayment or waiver is made cannot be earlier than client's transfer date to this office";
          action = "repayment.or.waiver";
          postfix = "cannot.be.made.before.client.transfer.date";
          break;
        case LOAN_REJECTED:
          errorMessage =
              "The date on which a loan is rejected cannot be earlier than client's transfer date to this office";
          action = "reject";
          postfix = "cannot.be.before.client.transfer.date";
          break;
        case LOAN_WITHDRAWN:
          errorMessage =
              "The date on which a loan is withdrawn cannot be earlier than client's transfer date to this office";
          action = "withdraw";
          postfix = "cannot.be.before.client.transfer.date";
          break;
        case WRITE_OFF_OUTSTANDING:
          errorMessage =
              "The date on which a write off is made cannot be earlier than client's transfer date to this office";
          action = "writeoff";
          postfix = "cannot.be.undone.before.client.transfer.date";
          break;
        case REPAID_IN_FULL:
          errorMessage =
              "The date on which the loan is repaid in full cannot be earlier than client's transfer date to this office";
          action = "close";
          postfix = "cannot.be.undone.before.client.transfer.date";
          break;
        case LOAN_CHARGE_PAYMENT:
          errorMessage =
              "The date on which a charge payment is made cannot be earlier than client's transfer date to this office";
          action = "charge.payment";
          postfix = "cannot.be.made.before.client.transfer.date";
          break;
        case LOAN_REFUND:
          errorMessage =
              "The date on which a refund is made cannot be earlier than client's transfer date to this office";
          action = "refund";
          postfix = "cannot.be.made.before.client.transfer.date";
          break;
  ...
  private void
  validateActivityNotBeforeLastTransactionDate(final LoanEvent event,
    ...
    if (lastTransactionDate.isAfter(activityDate)) {
      ...
      case LOAN_REPAYMENT_OR_WAIVER:
        errorMessage =
            "The date on which a repayment or waiver is made cannot be earlier than last transaction date";
        action = "repayment.or.waiver";
      ...
      case LOAN_CHARGE_PAYMENT:
        errorMessage =
            "The date on which a charge payment is made cannot be earlier than last transaction date";
        action = "charge.payment";
        postfix = "cannot.be.made.before.last.transaction.date";
        break;
      default:
  ...
  public List<LoanInstallmentCharge>
  generateInstallmentLoanCharges(final LoanCharge loanCharge) {
    final List<LoanInstallmentCharge> loanChargePerInstallments =
        new ArrayList<>();
    if (loanCharge.isInstalmentFee()) {
      List<LoanRepaymentScheduleInstallment> installments =
          getRepaymentScheduleInstallments();
  ...
  public void validateAccountStatus(final LoanEvent event) {
    ...
    case LOAN_APPROVED:
      if (!isSubmittedAndPendingApproval()) {
        final String defaultUserMessage =
            "Loan Account Approval is not allowed. Loan Account is not in submitted and pending approval state.";
    ...
    case LOAN_DISBURSAL_UNDO:
      ...
      if (isOpen() && this.isTopup()) {
        final String defaultUserMessage =
            "Loan Undo disbursal is not allowed on Topup Loans";
        final ApiParameterError error = ApiParameterError.generalError(
    ...
    case LOAN_REJECTED:
      if (!isSubmittedAndPendingApproval()) {
        final String defaultUserMessage =
            "Loan application cannot be rejected. Loan Account is not in Submitted and Pending approval state.";
        final ApiParameterError error = ApiParameterError.generalError(
    ...
    case LOAN_WITHDRAWN:
      if (!isSubmittedAndPendingApproval()) {
        final String defaultUserMessage =
            "Loan application cannot be withdrawn. Loan Account is not in Submitted and Pending approval state.";
        final ApiParameterError error = ApiParameterError.generalError(
  ...
  public ChangedTransactionDetail updateDisbursementDateAndAmountForTranche(
    ...
    disbursementDetails.updateExpectedDisbursementDateAndAmount(
        Date.from(expectedDisbursementDate.atStartOfDay(ZoneId.systemDefault())
                      .toInstant()),
        principal);
  ...
  public BigDecimal retriveLastEmiAmount() {
    ...
    Date startDate = Date.from(this.getDisbursementDate()
                                   .atStartOfDay(ZoneId.systemDefault())
                                   .toInstant());
  ...
  public BigDecimal getApprovedPrincipal() { return this.approvedPrincipal; }

  public BigDecimal getTotalOverpaid() { return this.totalOverpaid; }
  ...
  public LocalDate getExpectedMaturityDate() {
    ...
    if (this.expectedMaturityDate != null) {
      expectedMaturityDate = LocalDate.ofInstant(
          this.expectedMaturityDate.toInstant(), ZoneId.systemDefault());
    }
  ...
  public LocalDate getMaturityDate() {
    ...
    if (this.actualMaturityDate != null) {
      maturityDate = LocalDate.ofInstant(this.actualMaturityDate.toInstant(),
                                         ZoneId.systemDefault());
    }
    return maturityDate;
  }
  ...
  public ChangedTransactionDetail processTransactions() {
     ...
     * transactions. for this need to save the reversed transactions first and
  ...
  private void
  updateLoanChargesPaidBy(LoanTransaction accrual,
                          HashMap<String, Object> feeDetails,
                          LoanRepaymentScheduleInstallment installment) {
    @SuppressWarnings("unchecked")
    List<LoanCharge> loanCharges =
        (List<LoanCharge>)feeDetails.get("loanCharges");
    @SuppressWarnings("unchecked")
    ...
    if (loanCharges != null) {
      for (LoanCharge loanCharge : loanCharges) {
        Integer installmentNumber =
            null == installment ? null : installment.getInstallmentNumber();
        final LoanChargePaidBy loanChargePaidBy = new LoanChargePaidBy(
            accrual, loanCharge,
            loanCharge.getAmount(getCurrency()).getAmount(), installmentNumber);
  ...
  public void processIncomeTransactions(AppUser currentUser) {
    if (this.loanInterestRecalculationDetails != null &&
      ...
      List<LoanTransaction> incomeTransactions =
          retreiveListOfIncomePostingTransactions();
      List<LoanTransaction> accrualTransactions =
          retreiveListOfAccrualTransactions();
      for (LoanInterestRecalcualtionAdditionalDetails compoundingDetail :
           compoundingDetails) {
        ...
        LoanTransaction incomeTransaction = getTransactionForDate(
            incomeTransactions, compoundingDetail.getEffectiveDate());
        LoanTransaction accrualTransaction = getTransactionForDate(
            accrualTransactions, compoundingDetail.getEffectiveDate());
        addUpdateIncomeAndAccrualTransaction(
            compoundingDetail, lastCompoundingDate, currentUser,
  ...
  private void reverseTransactionsOnOrAfter(List<LoanTransaction> transactions,
                                            Date date) {
    LocalDate refDate =
  ...
  private void addUpdateIncomeAndAccrualTransaction(
    ...
    if (this.loanInterestRecalculationDetails
      ...
      penalties = (BigDecimal)feeDetails.get("penalties");
                   ...
                   .getInterestRecalculationCompoundingMethod()
                   .equals(InterestRecalculationCompoundingMethod
                               .INTEREST_AND_FEE)) {
      determineFeeDetails(lastCompoundingDate,
                          compoundingDetail.getEffectiveDate(), feeDetails);
      fee = (BigDecimal)feeDetails.get("fee");
      penalties = (BigDecimal)feeDetails.get("penalties");
      interest =
          compoundingDetail.getAmount().subtract(fee).subtract(penalties);
    }

    if (existingIncomeTransaction == null) {
      LoanTransaction transaction = LoanTransaction.incomePosting(
          this, this.getOffice(),
          Date.from(compoundingDetail.getEffectiveDate()
                        .atStartOfDay(ZoneId.systemDefault())
                        .toInstant()),
    ...
    if (isPeriodicAccrualAccountingEnabledOnLoanProduct()) {
      if (existingAccrualTransaction == null) {
        LoanTransaction accrual = LoanTransaction.accrueTransaction(
            this, this.getOffice(), compoundingDetail.getEffectiveDate(),
  ...
  getTransactionForDate(List<LoanTransaction> transactions,
  ...
  private void
  reverseTransactionsPostEffectiveDate(List<LoanTransaction> transactions,
                                       LocalDate effectiveDate) {
    for (LoanTransaction loanTransaction : transactions) {
  ...
  extractInterestRecalculationAdditionalDetails() {
    List<LoanInterestRecalcualtionAdditionalDetails> retDetails =
        new ArrayList<>();
    List<LoanRepaymentScheduleInstallment> repaymentSchedule =
        getRepaymentScheduleInstallments();
  ...
  getRecalculatedSchedule(final ScheduleGeneratorDTO generatorDTO) {
    ...
    return loanScheduleGenerator.rescheduleNextInstallments(
        mc, loanApplicationTerms, this, generatorDTO.getHolidayDetailDTO(),
        loanRepaymentScheduleTransactionProcessor,
        generatorDTO.getRecalculateFrom());
  }

  public LoanRepaymentScheduleInstallment
  fetchPrepaymentDetail(final ScheduleGeneratorDTO scheduleGeneratorDTO,
  ...
  public LoanApplicationTerms constructLoanApplicationTerms(
    ...
    if (calendar != null) {
      nthDayType =
          CalendarUtils.getRepeatsOnNthDayOfMonth(calendar.getRecurrence());
      dayOfWeekType = DayOfWeekType.fromInt(
          CalendarUtils.getRepeatsOnDay(calendar.getRecurrence()).getValue());
    }
    HolidayDetailDTO holidayDetailDTO =
        scheduleGeneratorDTO.getHolidayDetailDTO();
    ...
    if (this.repaymentScheduleDetail().isInterestRecalculationEnabled()) {
      ...
      allowCompoundingOnEod =
          this.loanInterestRecalculationDetails.allowCompoundingOnEod();
    ...
    final LoanApplicationTerms loanApplicationTerms =
        LoanApplicationTerms.assembleFrom(
            scheduleGeneratorDTO.getApplicationCurrency(), loanTermFrequency,
            loanTermPeriodFrequencyType, nthDayType, dayOfWeekType,
            getDisbursementDate(), getExpectedFirstRepaymentOnDate(),
            scheduleGeneratorDTO.getCalculatedRepaymentsStartingFromDate(),
            getInArrearsTolerance(), this.loanRepaymentScheduleDetail,
            this.loanProduct.isMultiDisburseLoan(), this.fixedEmiAmount,
            disbursementData, this.maxOutstandingLoanBalance,
  ...
  public BigDecimal
  constructLoanTermVariations(FloatingRateDTO floatingRateDTO,
                              BigDecimal annualNominalInterestRate,
  ...
  private LoanRepaymentScheduleInstallment getTotalOutstandingOnLoan() {
    ...
    for (final LoanRepaymentScheduleInstallment scheduledRepayment :
      ...
      totalPrincipal = totalPrincipal.plus(
          scheduledRepayment.getPrincipalOutstanding(loanCurrency()));
      totalInterest = totalInterest.plus(
          scheduledRepayment.getInterestOutstanding(loanCurrency()));
      feeCharges = feeCharges.plus(
          scheduledRepayment.getFeeChargesOutstanding(loanCurrency()));
      penaltyCharges = penaltyCharges.plus(
          scheduledRepayment.getPenaltyChargesOutstanding(loanCurrency()));
    }
    return new LoanRepaymentScheduleInstallment(
        null, 0, LocalDate.now(ZoneId.systemDefault()),
        LocalDate.now(ZoneId.systemDefault()), totalPrincipal.getAmount(),
  ...
  public LocalDate getAccruedTill() {
    ...
    if (this.accruedTill != null) {
      accruedTill = LocalDate.ofInstant(this.accruedTill.toInstant(),
                                        ZoneId.systemDefault());
    }
    return accruedTill;
  }
  ...
  public LocalDate fetchInterestRecalculateFromDate() {
    ...
    if (this.interestRecalculatedOn == null) {
      ...
      interestRecalculatedOn = LocalDate.ofInstant(
          this.interestRecalculatedOn.toInstant(), ZoneId.systemDefault());
    }
  ...
  private void updateLoanOutstandingBalaces() {
    ...
    List<LoanTransaction> loanTransactions =
        retreiveListOfTransactionsExcludeAccruals();
   ...
   **/
  public BigDecimal getFixedEmiAmount() { return this.fixedEmiAmount; }
   ...
   **/
  public LoanRepaymentScheduleInstallment
  getRepaymentScheduleInstallment(LocalDate dueDate) {
    LoanRepaymentScheduleInstallment installment = null;
  ...
  public List<DisbursementData> getDisbursmentData() {
    Iterator<LoanDisbursementDetails> iterator =
        this.getDisbursementDetails().iterator();
    ...
    while (iterator.hasNext()) {
      ...
      if (loanDisbursementDetails.expectedDisbursementDate() != null) {
        expectedDisbursementDate = LocalDate.ofInstant(
            loanDisbursementDetails.expectedDisbursementDate().toInstant(),
            ZoneId.systemDefault());
      }

      if (loanDisbursementDetails.actualDisbursementDate() != null) {
        actualDisbursementDate = LocalDate.ofInstant(
            loanDisbursementDetails.actualDisbursementDate().toInstant(),
            ZoneId.systemDefault());
      }
      BigDecimal waivedChargeAmount = null;
      disbursementData.add(new DisbursementData(
          loanDisbursementDetails.getId(), expectedDisbursementDate,
          actualDisbursementDate, loanDisbursementDetails.principal(), null,
  ...
  public LoanApplicationTerms getLoanApplicationTerms(
    ...
    final MonetaryCurrency currency =
        this.loanRepaymentScheduleDetail.getCurrency();
    ...
    if (loanCalendar != null) {
      nthDayType =
          CalendarUtils.getRepeatsOnNthDayOfMonth(loanCalendar.getRecurrence());
    ...
    final Integer numberOfRepayments =
        this.loanRepaymentScheduleDetail.getNumberOfRepayments();
    ...
    final Integer graceOnArrearsAgeing =
        this.loanRepaymentScheduleDetail.getGraceOnDueDate();

    final Money inArrearsToleranceMoney =
        this.loanRepaymentScheduleDetail.getInArrearsTolerance();
    ...
    if (loanCalendar != null) {
      ...
      calendarHistoryDataWrapper =
          new CalendarHistoryDataWrapper(calendarHistory);
    }
    ...
    return LoanApplicationTerms.assembleFrom(
        applicationCurrency, loanTermFrequency, loanTermPeriodFrequencyType,
        nthDayType, dayOfWeekType, expectedDisbursementDate,
        repaymentsStartingFromDate, calculatedRepaymentsStartingFromDate,
        inArrearsToleranceMoney, this.loanRepaymentScheduleDetail,
        ...
        holidayDetailDTO, allowCompoundingOnEod, false, false, firstSemiDate,
        secondSemiDate);
  }
   ...
   **/
  public LoanSummary getLoanSummary() { return this.summary; }
  ...
  public void updateNumberOfRepayments(Integer numberOfRepayments) {
    this.loanRepaymentScheduleDetail.updateNumberOfRepayments(
        numberOfRepayments);
  }
  ...
  public void updateRescheduledOnDate(LocalDate rescheduledOnDate) {
    ...
    if (rescheduledOnDate != null) {
      this.rescheduledOnDate = Date.from(
          rescheduledOnDate.atStartOfDay(ZoneId.systemDefault()).toInstant());
    }
  }
  ...
  public ChangedTransactionDetail makeRefundForActiveLoan(
    ...
    final ChangedTransactionDetail changedTransactionDetail =
        handleRefundTransaction(loanTransaction, loanLifecycleStateMachine,
                                null);
  ...
  private ChangedTransactionDetail handleRefundTransaction(
    ...
    if (status().isOverpaid() || status().isClosed()) {
      ...
      final String errorMessage =
          "This refund option is only for active loans ";
      throw new InvalidLoanStateTransitionException(
          "transaction", "is.exceeding.overpaid.amount", errorMessage,
          this.totalOverpaid,
          loanTransaction.getAmount(getCurrency()).getAmount());
    ...
    if (loanTransaction.isNotRefundForActiveLoan()) {
      final String errorMessage =
          "A transaction of type refund was expected but not received.";
      throw new InvalidLoanTransactionTypeException(
  ...
  private Date getActualDisbursementDate(final LoanCharge loanCharge) {
    ...
    if (loanCharge.isDueAtDisbursement() && loanCharge.isActive()) {
      ...
      if (trancheDisbursementCharge != null) {
        LoanDisbursementDetails details =
            trancheDisbursementCharge.getloanDisbursementDetails();
  ...
  public Map<String, Object>
  undoLastDisbursal(ScheduleGeneratorDTO scheduleGeneratorDTO,
                    List<Long> existingTransactionIds,
                    List<Long> existingReversedTransactionIds,
                    AppUser currentUser, Loan loan) {
    ...
    for (final LoanTransaction previousTransaction : loanTransactions) {
      if (lastTransactionDate.isBefore(
              previousTransaction.getTransactionDate())) {
        if (previousTransaction.isRepayment() ||
    ...
    for (Iterator<LoanTermVariations> iterator =
             this.loanTermVariations.iterator();
         iterator.hasNext();) {
      ...
      if ((loanTermVariations.getTermType().isDueDateVariation() &&
          ...
          (loanTermVariations.getTermType().isEMIAmountVariation() &&
                   loanTermVariations.getTermApplicableFrom().compareTo(
                       Date.from(actualDisbursementDate
                                     .atStartOfDay(ZoneId.systemDefault())
                                     .toInstant())) == 0
               ? Boolean.TRUE
               : Boolean.FALSE) ||
          loanTermVariations.getTermApplicableFrom().after(Date.from(
              actualDisbursementDate.atStartOfDay(ZoneId.systemDefault())
                  .toInstant()))) {
        iterator.remove();
      }
  ...
  private void
  updateLoanToLastDisbursalState(LocalDate actualDisbursementDate) {
      ...
      if (charge.isOverdueInstallmentCharge()) {
        charge.setActive(false);
                 ...
                 actualDisbursementDate.equals(
                     LocalDate.ofInstant(charge.getTrancheDisbursementCharge()
                                             .getloanDisbursementDetails()
                                             .actualDisbursementDate()
                                             .toInstant(),
                                         ZoneId.systemDefault()))) {
    ...
    for (final LoanDisbursementDetails details : this.disbursementDetails) {
      if (actualDisbursementDate.equals(
              LocalDate.ofInstant(details.actualDisbursementDate().toInstant(),
                                  ZoneId.systemDefault()))) {
        this.loanRepaymentScheduleDetail.setPrincipal(
  ...
  public void
  setExpectedFirstRepaymentOnDate(Date expectedFirstRepaymentOnDate) {
      ...
      if (loanDisbursementDetail.actualDisbursementDate() == null) {
        ...
        for (final LoanRepaymentScheduleInstallment installment :
          ...
          if (installment.getDueDate().isEqual(
              ...
              (installment.getDueDate().isAfter(
                   loanDisbursementDetail
                       .expectedDisbursementDateAsLocalDate()) &&
  ...
  public BigDecimal getDerivedAmountForCharge(final LoanCharge loanCharge) {
    BigDecimal amount = BigDecimal.ZERO;
    if (isMultiDisburmentLoan() &&
        loanCharge.getCharge().getChargeTimeType().equals(
            ChargeTimeType.DISBURSEMENT.getValue())) {
      ...
      if (loanCharge.isSpecifiedDueDate() && this.isMultiDisburmentLoan()) {
        for (final LoanDisbursementDetails loanDisbursementDetails :
             this.getDisbursementDetails()) {
          if (!loanDisbursementDetails.expectedDisbursementDate().after(
  ...
  fetchLoanForeclosureDetail(final LocalDate closureDate) {
    ...
    Money totalPrincipal = Money.of(
        getCurrency(), this.getSummary().getTotalPrincipalOutstanding());
    totalPrincipal = totalPrincipal.minus(receivables[3]);
    final Set<LoanInterestRecalcualtionAdditionalDetails> compoundingDetails =
        null;
    final LocalDate currentDate = DateUtils.getLocalDateOfTenant();
    return new LoanRepaymentScheduleInstallment(
        null, 0, currentDate, currentDate, totalPrincipal.getAmount(),
  ...
  public Money[] retriveIncomeOutstandingTillDate(final LocalDate paymentDate) {
    ...
    for (final LoanRepaymentScheduleInstallment installment :
      ...
      if (!installment.getDueDate().isAfter(paymentDate)) {
        ...
        if (balancesForCurrentPeroid[0].isGreaterThan(
                balancesForCurrentPeroid[5])) {
        ...
        if (balancesForCurrentPeroid[1].isGreaterThan(
                balancesForCurrentPeroid[3])) {
          fee = fee.plus(
              balancesForCurrentPeroid[1].minus(balancesForCurrentPeroid[3]));
        } else {
          paidFromFutureInstallments = paidFromFutureInstallments.plus(
              balancesForCurrentPeroid[3].minus(balancesForCurrentPeroid[1]));
        }
        if (balancesForCurrentPeroid[2].isGreaterThan(
                balancesForCurrentPeroid[4])) {
          penalty = penalty.plus(
              balancesForCurrentPeroid[2].minus(balancesForCurrentPeroid[4]));
        } else {
          paidFromFutureInstallments =
  ...
  private Money[] fetchInterestFeeAndPenaltyTillDate(
    ...
    int totalPeriodDays = Math.toIntExact(ChronoUnit.DAYS.between(
        installment.getFromDate(), installment.getDueDate()));
    int tillDays = Math.toIntExact(
        ChronoUnit.DAYS.between(installment.getFromDate(), paymentDate));
    interestForCurrentPeriod =
        Money.of(getCurrency(),
                 BigDecimal.valueOf(calculateInterestForDays(
                     totalPeriodDays,
                     installment.getInterestCharged(getCurrency()).getAmount(),
                     tillDays)));
    interestAccountedForCurrentPeriod =
        installment.getInterestWaived(getCurrency())
      ...
      if (loanCharge.isActive() && !loanCharge.isDueAtDisbursement()) {
        if (loanCharge.isDueForCollectionFromAndUpToAndIncluding(
          ...
          LoanInstallmentCharge loanInstallmentCharge =
              loanCharge.getInstallmentLoanCharge(
                  installment.getInstallmentNumber());
          if (loanCharge.isPenaltyCharge()) {
            penaltyAccoutedForCurrentPeriod =
                penaltyAccoutedForCurrentPeriod.plus(
                    loanInstallmentCharge.getAmountPaid(currency));
          } else {
            feeAccountedForCurrentPeriod = feeAccountedForCurrentPeriod.plus(
                loanInstallmentCharge.getAmountPaid(currency));
          }
  ...
  public Money[] retriveIncomeForOverlappingPeriod(
    ...
    for (final LoanRepaymentScheduleInstallment installment :
      ...
      if (installment.getDueDate().isEqual(paymentDate)) {
        ...
        balances = fetchInterestFeeAndPenaltyTillDate(paymentDate, currency,
                                                      installment);
        break;
      }
    }
  ...
  private double calculateInterestForDays(int daysInPeriod, BigDecimal interest,
                                          int days) {
    ...
    for (final LoanTransaction transaction : this.loanTransactions) {
      if (transaction.isNotReversed() &&
        ...
        if (transaction.isAccrual()) {
          ...
          receivablePenalty = receivablePenalty.plus(
          ...
          receivablePenalty = receivablePenalty.minus(
              transaction.getPenaltyChargesPortion(currency));
        }
      }
      if (receivableInterest.isLessThanZero()) {
  ...
  public ChangedTransactionDetail handleForeClosureTransactions(
      ...
      final LoanLifecycleStateMachine loanLifecycleStateMachine,
      final ScheduleGeneratorDTO scheduleGeneratorDTO, final AppUser appUser) {
  ...
  public Money retrieveAccruedAmountAfterDate(final LocalDate tillDate) {
    ...
    for (final LoanRepaymentScheduleInstallment installment :
         this.repaymentScheduleInstallments) {
      totalAmountAccrued = totalAmountAccrued.plus(
          installment.getInterestAccrued(getCurrency()));

      if (tillDate.isAfter(installment.getFromDate()) &&
          tillDate.isBefore(installment.getDueDate())) {
        int daysInPeriod = Math.toIntExact(ChronoUnit.DAYS.between(
            installment.getFromDate(), installment.getDueDate()));
        int tillDays = Math.toIntExact(
            ChronoUnit.DAYS.between(installment.getFromDate(), tillDate));
        double interest = calculateInterestForDays(
            daysInPeriod,
            installment.getInterestCharged(getCurrency()).getAmount(),
            tillDays);
        ...
        actualAmountTobeAccrued = actualAmountTobeAccrued.plus(
            installment.getInterestAccrued(getCurrency()));
      }
    }
  ...
  public void validateForForeclosure(final LocalDate transactionDate) {
    ...
    if (DateUtils.isDateInTheFuture(transactionDate)) {
      final String defaultUserMessage =
          "The transactionDate cannot be in the future.";
      throw new LoanForeclosureException(
          "loan.foreclosure.transaction.date.is.in.future", defaultUserMessage,
          transactionDate);
    }
  ...
  public void updateInstallmentsPostDate(LocalDate transactionDate) {
    ...
    for (LoanDisbursementDetails loanDisbursementDetails :
      ...
      if (loanDisbursementDetails.actualDisbursementDate() == null) {
        totalPrincipal =
            Money.of(currency, totalPrincipal.getAmount().subtract(
    ...
    if (newInstallments.size() > 0) {
      installmentStartDate =
          newInstallments.get(newInstallments.size() - 1).getDueDate();
    }
      ...
      if (loanTransaction.isChargesWaiver()) {
        for (LoanChargePaidBy chargePaidBy :
          ...
          if ((chargePaidBy.getLoanCharge().isDueDateCharge() &&
               chargePaidBy.getLoanCharge().getDueLocalDate().isAfter(
                   transactionDate)) ||
  ...
  public Integer getLoanSubStatus() { return this.loanSubStatus; }
  ...
  public void setIsTopup(final boolean isTopup) { this.isTopup = isTopup; }

  public boolean isTopup() { return this.isTopup; }
  ...
  public Collection<LoanCharge> getLoanCharges() { return this.charges; }
  ...
  public void initializeLoanOfficerHistory() { this.loanOfficerHistory.size(); }

  public void initilizeTransactions() { this.loanTransactions.size(); }
