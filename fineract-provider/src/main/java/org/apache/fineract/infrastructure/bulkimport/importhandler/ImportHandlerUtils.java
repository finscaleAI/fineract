...
public final class ImportHandlerUtils {
  ...
  public static Long readAsLong(int colIndex, Row row) {
    ...
    FormulaEvaluator eval = row.getSheet()
                                .getWorkbook()
                                .getCreationHelper()
  ...
  public static LocalDate readAsDate(int colIndex, Row row) {
    ...
    LocalDate localDate = LocalDate.ofInstant(c.getDateCellValue().toInstant(),
  ...
  public static CellStyle getCellStyle(Workbook workbook, IndexedColors color) {
    ...
    workbook.setSheetVisibility(workbook.getSheetIndex(cache),
  ...
  getDefaultUserMessages(List<ApiParameterError> ApiParameterErrorList) {
    StringBuilder defaultUserMessages = new StringBuilder();
    for (ApiParameterError error : ApiParameterErrorList) {
      defaultUserMessages =
          defaultUserMessages.append(error.getDefaultUserMessage() + '\t');
    }
  ...
  public static void writeErrorMessage(Sheet sheet, Integer rowIndex,
                                       String errorMessage, int statusColumn) {
    Cell statusCell = sheet.getRow(rowIndex).createCell(statusColumn);
    statusCell.setCellValue(errorMessage);
    statusCell.setCellStyle(
        getCellStyle(sheet.getWorkbook(), IndexedColors.RED));
  }

  public static String getErrorMessage(RuntimeException re) {
    if (re instanceof AbstractPlatformException) {
      AbstractPlatformException abstractPlatformException =
          (AbstractPlatformException)re;
      ...
      return getErrorList(
          unsupportedParameterException.getUnsupportedParameters());
    } else {
        ...
        for (Cell cell : row) {
          if (name != null) {
            if (cell.getCellType() == CellType.STRING &&
              ...
              if (sheetName.equals(
                      TemplatePopulateImportConstants.OFFICE_SHEET_NAME) ||
                  sheetName.equals(
                  ...
                  sheetName.equals(TemplatePopulateImportConstants
                                       .SHARED_PRODUCTS_SHEET_NAME) ||
                  sheetName.equals(
                      TemplatePopulateImportConstants.ROLES_SHEET_NAME)) {
                if (row.getCell(cell.getColumnIndex() - 1).getCellType() ==
                    CellType.NUMERIC) {
                  return ((Double)row.getCell(cell.getColumnIndex() - 1)
                              .getNumericCellValue())
                      .longValue();
                }
                return 0L;
              } else if (sheetName.equals(TemplatePopulateImportConstants
                                              .CLIENT_SHEET_NAME) ||
                         sheetName.equals(TemplatePopulateImportConstants
                                              .CENTER_SHEET_NAME) ||
                         sheetName.equals(TemplatePopulateImportConstants
      ...
      for (Row row : sheet) {
        for (Cell cell : row) {
          if (name != null) {
            if (cell.getCellType() == CellType.STRING &&
                cell.getRichStringCellValue().getString().trim().equals(name)) {
              chargeTimeType = row.getCell(cell.getColumnIndex() + 3)
                                   .getStringCellValue()
                                   .toString();
            }
          }
  ...
  getChargeAmountTypeEnum(final String chargeAmountType) {
  ...
  public static String getRepeatsOnDayId(String repeatsOnDay) {
    if (repeatsOnDay != null) {
      if (repeatsOnDay.equalsIgnoreCase(
              TemplatePopulateImportConstants.MONDAY)) {
        repeatsOnDay = "1";
      } else if (repeatsOnDay.equalsIgnoreCase(
