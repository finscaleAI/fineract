...
public class LoanWithSemiMonthlyRepaymentScheduleTest {
  ...
  public void testLoanRepaymentPerSemiMonth() {
    ...
    final Integer clientID = ClientHelper.createClient(
        this.requestSpec, this.responseSpec, "01 January 2014");
    LOG.info(
        "---------------------------------CLIENT CREATED WITH ID--------------------------------------------------- {}",
        clientID);

    // CREATE LOAN MULTIDISBURSAL PRODUCT WITH SEMI MONTH REPAYMENT
    final Integer loanProductID = createLoanProductWithSemiMonthlyRepayment(
        LoanProductTestBuilder.RBI_INDIA_STRATEGY, firstSemiDate,
        secondSemiDate, null, null);

    // APPLY FOR LOAN WITH SEMI MONTHREPAYMENT
    final Integer loanID = applyForLoanApplicationWithSemiMonthlyRepayment(
        clientID, loanProductID, disbursementDate);
    ...
    LOG.info(
        "-----------------------------------APPROVE LOAN-----------------------------------------------------------");
    loanStatusHashMap =
        this.loanTransactionHelper.approveLoan(approveDate, loanID);
    ...
    loanStatusHashMap = LoanStatusChecker.getStatusOfLoan(
        this.requestSpec, this.responseSpec, loanID);
  ...
  private void
  verifyLoanRepaymentSchedule(final HashMap lastInstallment,
    ...
    assertEquals(String.valueOf(expectedvalues.get("principalDue")),
                 String.valueOf(lastInstallment.get("principalDue")));
    assertEquals(String.valueOf(expectedvalues.get("interestDue")),
                 String.valueOf(lastInstallment.get("interestDue")));
  }

  private Integer createLoanProductWithSemiMonthlyRepayment(
      final String repaymentStrategy, final String firstSemiDate,
      final String secondSemiDate, final Account[] accounts,
    ...
    LoanProductTestBuilder builder =
        new LoanProductTestBuilder()
            ...
            .withRepaymentTypeAsSemiMonth(firstSemiDate, secondSemiDate)
  ...
  private Integer applyForLoanApplicationWithSemiMonthlyRepayment(
